...
export interface RulesAndSkillsAPI {
  listRules: (
    projectRoot: string,
  ) => Promise<{ success: boolean; rules?: RulesFile[]; error?: string }>;
  readRule: (
    projectRoot: string,
  ...
  listCommands: (
    projectRoot?: string,
  ) => Promise<{ success: boolean; commands?: CommandDefinition[]; error?: string }>;
  createCommand: (
    args: CmdCreateArgs,
  ) => Promise<{ success: boolean; filePath?: string; error?: string }>;
  readCommand: (
  ...
  listRuleFiles: (
    projectRoot?: string,
  ) => Promise<{ success: boolean; ruleFiles?: RuleDefinition[]; error?: string }>;
  createRuleFile: (args: {
    scope: string;
    name: string;
    content: string;
  ...
  getHooksConfig: (
    ...
    projectRoot?: string,
  ) => Promise<{ success: boolean; hooks?: HooksConfig; error?: string }>;
  addHook: (args: {
    scope: 'global' | 'project';
    eventType: string;
    command: string;
