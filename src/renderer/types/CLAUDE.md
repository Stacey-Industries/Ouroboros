...
- **ClaudeCliSettings duplication**: type lives in both `electron-foundation.d.ts` AND `src/main/configTypes.ts` until Wave 97 unifies them in `src/shared/types/`. New fields in main must be mirrored to the renderer copy. Reason: Wave 96 cut renderer→main type import to fix tsc:web TS6307 cascade; full extraction deferred.
