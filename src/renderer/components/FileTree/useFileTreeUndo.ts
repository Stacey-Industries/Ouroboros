...
function toastRestoreResult(
  ...
  if (failed === 0) {
    toast(succeeded === 1 ? `Restored "${items[0].name}"` : `Restored ${succeeded} items`);
  } else if (succeeded === 0) {
    toast('Undo failed — files may have been permanently removed', 'error');
  } else {
    toast(`Restored ${succeeded} of ${items.length} items (${failed} failed)`, 'warning');
  }
}
...
export function useFileTreeUndo(refreshDir: (dir: string) => void, toast: ToastFn): FileTreeUndoResult {
  ...
  const undo = useCallback(() => {
    ...
    void executeUndo(items, refreshDir, toast).catch(() => {
