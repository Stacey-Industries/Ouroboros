...
Now I have enough context to write the CLAUDE.md. Let me produce it:

```markdown
# FileTree — Multi-Root Hierarchical File Browser

VS Code-style file tree with git integration, virtual scrolling, inline rename, context menus, file nesting, and a Zustand store that is being incrementally migrated away from local React state.
...
| `fileTypeIcons.tsx` | Language → SVG icon map |
...
When adding new cross-cutting UI state, put it in the store. When adding per-root data that varies by directory path, evaluate whether the store's `Map<string, …>` pattern is appropriate.
