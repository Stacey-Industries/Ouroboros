  ...
  return (
    <li className="flex items-center gap-2 text-xs text-text-semantic-muted">
      <span
        ...
        style={{ background: criticalityColour(entry.criticality) }}
        aria-hidden
      />
      <span className="truncate font-mono">{entry.node.name}</span>
      <span className="ml-auto shrink-0 text-[10px] text-text-semantic-faint">
        d={entry.distance}
      </span>
    </li>
  );
...
function OfferActions({ onAccept, onDismiss }: OfferActionsProps): React.ReactElement {
  return (
    <div className="flex gap-2">
      <button
        className="rounded bg-interactive-accent px-2.5 py-1 text-xs font-medium text-text-on-accent hover:bg-interactive-hover transition-colors"
        onClick={onAccept}
      >
        Accept
      </button>
      <button
        className="rounded px-2.5 py-1 text-xs text-text-semantic-muted hover:text-text-semantic-primary transition-colors"
        onClick={onDismiss}
      >
        Dismiss
      </button>
...
function CallerList({ callers }: { callers: BlastRadiusEntry[] }): React.ReactElement {
  return (
    <ul className="flex flex-col gap-0.5">
      {callers.map((entry) => (
        <CallerRow key={entry.node.id} entry={entry} />
      ))}
    </ul>
  );
}

export function BlastRadiusPinOffer({
  callers,
  maxDisplay = 3,
  onAccept,
  onDismiss,
  enabled,
}: BlastRadiusPinOfferProps): React.ReactElement | null {
  ...
  return (
    <div
      className="flex flex-col gap-2 rounded-lg border border-border-accent bg-surface-raised px-3 py-2.5 shadow-lg"
      role="status"
      aria-live="polite"
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          className="shrink-0 text-text-semantic-faint hover:text-text-semantic-primary transition-colors text-sm leading-none"
          onClick={onDismiss}
          aria-label="Dismiss caller offer"
        >
          ×
        </button>
      </div>
      <CallerList callers={topCallers} />
      <OfferActions onAccept={() => onAccept(topCallers.map(toPinKey))} onDismiss={onDismiss} />
    </div>
  );
}
