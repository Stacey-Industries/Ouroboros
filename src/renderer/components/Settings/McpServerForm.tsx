...
export function McpServerForm({
  form, isEdit,
  onFieldChange, onScopeChange,
  onAddEnvRow, onRemoveEnvRow, onUpdateEnvRow,
  onSubmit, onCancel,
}: McpServerFormProps): React.ReactElement {
  return (
    <div style={formContainerStyle}>
      {!isEdit && <FormField label="Name" value={form.name} field="name" onChange={onFieldChange} placeholder="e.g., filesystem, github" autoFocus />}
      <FormField label="Command" value={form.command} field="command" onChange={onFieldChange} placeholder="e.g., npx, node, python" />
      <FormField label="Arguments (space-separated)" value={form.args} field="args" onChange={onFieldChange} placeholder="e.g., -y @modelcontextprotocol/server-filesystem /path" />
      <FormField label="URL (optional, for SSE/HTTP)" value={form.url} field="url" onChange={onFieldChange} placeholder="e.g., http://localhost:3001/sse" />
      <EnvRowsEditor rows={form.envRows} onAdd={onAddEnvRow} onRemove={onRemoveEnvRow} onUpdate={onUpdateEnvRow} />
...
function FormField({ label, value, field, onChange, placeholder, autoFocus }: {
  label: string; value: string; field: keyof ServerFormState;
...
function EnvRowsEditor({ rows, onAdd, onRemove, onUpdate }: {
...
function EnvRowInput({ row, idx, onUpdate, onRemove }: {
...
function ScopeSelector({ scope, onChange }: {
  scope: 'global' | 'project'; onChange: (s: 'global' | 'project') => void;
}): React.ReactElement {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
...
function ScopeRadio({ label, checked, onChange }: {
  label: string; checked: boolean; onChange: () => void;
}): React.ReactElement {
  return (
    <label style={radioLabelStyle}>
      <input type="radio" name="mcp-scope" checked={checked} onChange={onChange} style={{ accentColor: 'var(--accent)' }} />
      {label}
    </label>
  );
}

function FormActions({ isEdit, onSubmit, onCancel }: {
  isEdit: boolean; onSubmit: () => void; onCancel: () => void;
}): React.ReactElement {
  return (
