...
function ServerMetadataSection({
  server,
  pkg,
}: {
  server: McpRegistryServer;
          ...
          <div key={ev.name} style={envVarRowStyle}>
            ...
            <input
              type={
                ...
                ev.name.toLowerCase().includes('secret') ||
                ev.name.toLowerCase().includes('token')
                  ? 'password'
                  : 'text'
              }
              value={envValues[ev.name] ?? ''}
              onChange={(e) => onEnvChange(ev.name, e.target.value)}
