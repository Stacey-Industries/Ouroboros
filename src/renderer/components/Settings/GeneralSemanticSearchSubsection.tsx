...
function SemanticToggle({
  ...
  return (
    <div style={toggleRowStyle}>
      <div style={{ flex: 1 }}>
        <label className="text-text-semantic-primary" style={labelStyle}>
          Enable Semantic Search
        </label>
        <p className="text-text-semantic-faint" style={hintStyle}>
          Builds a vector index of your code.
        </p>
      </div>
...
export function SemanticSearchSubsection({ draft, onChange }: Props): React.ReactElement {
  ...
  return (
    <section style={{ marginTop: '24px' }}>
      ...
      <SemanticToggle enabled={enabled} onChange={(v) => onChange('embeddingsEnabled', v)} />
      {enabled && (
        <ProviderDropdown
          value={providerValue}
      ...
      {enabled && providerValue === 'voyage' && (
        <VoyageKeyInput value={voyageKey} onChange={(v) => onChange('voyageApiKey', v)} />
      )}
