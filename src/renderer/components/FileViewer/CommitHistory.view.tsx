...
function getPatchTone(line: string): React.CSSProperties {
  ...
  if (line.startsWith('@@')) {
    return { backgroundColor: 'rgba(88, 166, 255, 0.08)', color: 'var(--accent)' };
  }
...
const PatchHeader = memo(function PatchHeader({ onBack }: { onBack: () => void }): React.ReactElement {
  return (
    <div style={{ ...headerStyle, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      <button onClick={onBack} title="Back to commit list" style={{ ...buttonStyle, fontSize: '0.6875rem', padding: '2px 8px' }}>
        Back
      </button>
    </div>
  );
...
const CommitRow = memo(function CommitRow({
  ...
  return (
    <button
      ...
      onMouseEnter={(event) => { event.currentTarget.style.backgroundColor = 'var(--bg-secondary)'; }}
      onMouseLeave={(event) => { event.currentTarget.style.backgroundColor = 'transparent'; }}
    >
      <span style={{ flexShrink: 0, width: '52px', fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: 'var(--accent)' }}>
        {shortHash}
      </span>
...
const CommitListPanel = memo(function CommitListPanel({
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>File history - click a commit to view its diff</div>
      <div style={{ flex: 1, overflow: 'auto' }}>
        {commits.length === 0 && !isLoading && !error ? <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-faint)', fontSize: '0.8125rem' }}>No commits found for this file.</div> : null}
        {error ? <div style={{ padding: '24px', textAlign: 'center', color: 'var(--error)', fontSize: '0.8125rem' }}>{error}</div> : null}
        ...
        {hasMore && !isLoading && commits.length > 0 ? (
          <div style={{ padding: '8px 12px', textAlign: 'center' }}>
            <button onClick={() => void loadMore()} style={{ ...buttonStyle, fontSize: '0.75rem', padding: '4px 12px' }}>Load more</button>
          </div>
        ) : null}
        {isLoading ? <div style={{ padding: '16px', textAlign: 'center', color: 'var(--text-faint)', fontSize: '0.8125rem' }}>Loading...</div> : null}
      </div>
    </div>
  );
});
...
export const CommitHistoryView = memo(function CommitHistoryView({
  ...
  if (selectedHash !== null && patchError) {
    return <StatusMessage action={<button onClick={onBack} style={{ ...buttonStyle, fontSize: '0.75rem', padding: '3px 10px' }}>Back</button>} message={patchError} tone="var(--error)" />;
  }
