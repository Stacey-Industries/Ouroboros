...
function ImageContent({
  filePath,
  viewer,
}: Pick<ImageViewerFrameProps, 'filePath' | 'viewer'>): React.ReactElement {
  return (
    <ImageViewport
      fileUrl={viewer.fileUrl}
      filePath={filePath}
      loadError={viewer.loadError}
      fileError={viewer.fileError}
      imgRef={viewer.imgRef}
      onLoad={viewer.handleLoad}
      onError={viewer.handleError}
      zoomMode={viewer.zoomMode}
      imgStyle={getImageStyle(viewer.zoomMode, viewer.naturalWidth, viewer.naturalHeight, viewer.customZoom)}
...
export function ImageViewerFrame({
  filePath, fileSize, viewer, isSvg, showSource, onToggleSource,
}: ImageViewerFrameProps): React.ReactElement {
  const zoomLabel = getZoomLabel(viewer.zoomMode, viewer.customZoom);
  return (
    <div style={rootStyle}>
      <ImageViewerToolbar
        zoomMode={viewer.zoomMode} zoomLabel={zoomLabel}
        onFit={viewer.setFit} onActualSize={viewer.setActualSize}
        onZoomOut={viewer.zoomOut} onZoomIn={viewer.zoomIn}
        isSvg={isSvg} showSource={showSource} onToggleSource={onToggleSource}
      />
      <ImageContent filePath={filePath} viewer={viewer} />
      <ImageStatusBar naturalWidth={viewer.naturalWidth} naturalHeight={viewer.naturalHeight} fileSize={fileSize} zoomLabel={zoomLabel} />
    </div>
  );
}
