...
const COMPLETION_KIND_MAP: Record<string, monaco.languages.CompletionItemKind> = {
  ...
  color: CIK.Color,
  file: CIK.File,
  reference: CIK.Reference,
  folder: CIK.Folder,
  constant: CIK.Constant,
  struct: CIK.Struct,
  event: CIK.Event,
  operator: CIK.Operator,
  typeparameter: CIK.TypeParameter,
};
...
export function registerLspHoverProvider(): monaco.IDisposable {
  ...
  hoverReg = monaco.languages.registerHoverProvider({ pattern: '**' }, { provideHover });
...
async function provideDefinition(
  ...
  try {
    ...
    return {
      ...
      range: new monaco.Range(loc.line + 1, loc.character + 1, loc.line + 1, loc.character + 1),
    };
  } catch {
    return null;
  }
...
export function registerLspDefinitionProvider(): monaco.IDisposable {
  ...
  defReg = monaco.languages.registerDefinitionProvider({ pattern: '**' }, { provideDefinition });
