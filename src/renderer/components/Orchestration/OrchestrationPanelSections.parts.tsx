...
export function TaskStateBody(props: {
  ...
  return (
    <div className="mt-3 space-y-3 text-[12px]">
      ...
      <LatestMessageCard
        ...
        latestResultMessage={props.latestResultMessage}
      />
...
function SelectedContextCard({
  ...
  return (
    <KeyedBlock
      ...
      value={`${session?.contextPacket?.files.length ?? 0} file(s) in packet ${session?.contextPacket?.id ?? '—'}`}
    />
  );
}
  ...
  return (
    <KeyedBlock
      ...
      value={
        ...
        'No status message has been recorded.'
      }
      multiline
    />
  );
...
function buildSessionIdentityItems(
  session: TaskSessionRecord | null,
  ...
  return [
    { label: 'Session ID', value: pickString('—', session?.id), breakAll: true },
    {
      ...
      value: pickString('—', session?.providerSession?.sessionId),
      breakAll: true,
    },
    {
      label: 'Request ID',
      value: pickString('—', session?.providerSession?.requestId),
      breakAll: true,
    },
  ];
}
...
function buildSessionRuntimeItems(
  ...
  return [
    {
      label: 'Last context packet',
      value: pickString('—', session?.contextPacket?.id, latestAttempt?.contextPacketId),
      breakAll: true,
    },
...
export function SessionMemorySummary({
  ...
  return (
    <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3 text-[12px]">
      {items.map((item) => (
        <KeyValue key={item.label} label={item.label} value={item.value} breakAll={item.breakAll} />
      ))}
    </div>
  );
}

export function SessionMemoryLists({
  session,
}: {
  session: TaskSessionRecord | null;
}): React.ReactElement {
      ...
      <div className="mt-2 space-y-2">
        {issues.length ? (
          ...
          <div className="text-[12px]" style={{ color: 'var(--text-muted)' }}>
            No unresolved issues are saved for this session.
          </div>
        )}
      </div>
    </div>
...
function AttemptListCard({
  attempts,
}: {
  attempts: TaskSessionRecord['attempts'];
}): React.ReactElement {
  return (
    <div className="rounded-md border p-3" style={panelStyle('var(--bg)')}>
      <div className="text-[12px] font-semibold" style={{ color: 'var(--text)' }}>
      ...
      <div className="mt-2 space-y-2">
        {attempts.length ? (
          ...
          <div className="text-[12px]" style={{ color: 'var(--text-muted)' }}>
            No attempt timeline is available yet.
          </div>
        )}
      </div>
    </div>
...
function AttemptCard({
  ...
  attempt: TaskSessionRecord['attempts'][number];
}): React.ReactElement {
  ...
  return (
    <div className="rounded-md border px-3 py-2 text-[12px]" style={panelStyle()}>
      ...
      {attempt.resultMessage ? (
        <div className="mt-2 whitespace-pre-wrap" style={{ color: 'var(--text)' }}>
          {attempt.resultMessage}
        </div>
      ) : null}
    </div>
...
function KeyValue({
  ...
  return (
    <div>
      <div style={{ color: 'var(--text-muted)' }}>{label}</div>
      <div className={`mt-1 ${breakAll ? 'break-all' : ''}`} style={{ color: 'var(--text)' }}>
        {value}
      </div>
    </div>
  );
