...
import { badgeStyle, formatDateTime, resolveStatusTone } from './orchestrationUi';
...
function SessionHistoryHeader({
  session,
  tone,
}: {
  session: TaskSessionRecord;
  tone: ReturnType<typeof resolveStatusTone>;
}): React.ReactElement {
  return (
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        <div
          ...
          title={session.request.goal}
        >
          {trimGoal(session.request.goal)}
        </div>
...
function SessionHistoryMetaGrid({ session }: { session: TaskSessionRecord }): React.ReactElement {
  return (
...
export function SessionHistoryItem({
  ...
  return (
    ...
    >
      <SessionHistoryHeader session={session} tone={tone} />
      <SessionHistoryMetaGrid session={session} />
    </button>
  );
}
