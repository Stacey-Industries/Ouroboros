  ...
  return (
    <div className="flex items-start justify-between gap-3">
      <SectionIntro
        ...
        description="Latest execution state emitted by the provider adapter."
      />
...
export function ProviderDetails({
  ...
  return (
    <div className="mt-4 space-y-3 text-[12px]">
      <KeyValue
        label="Message"
        value={providerEvent?.message ?? 'No provider progress has been emitted yet.'}
        multiline
      />
...
function ProviderSessionGrid({
  providerEvent,
}: {
  providerEvent: ProviderProgressEvent;
}): React.ReactElement {
  return (
    <div className="grid gap-3 md:grid-cols-3">
      ...
      <KeyValue
        label="External task"
        value={providerEvent.session?.externalTaskId ?? '—'}
        breakAll
      />
    </div>
  );
...
export function VerificationStatusHeader({
  summary,
}: {
  ...
  return (
    <div className="flex items-start justify-between gap-3">
      <SectionIntro
        ...
        description="Command, diagnostics, and issue results captured for this orchestration session."
      />
...
function buildVerificationTimingItems(
  summary: VerificationSummary | null,
...
function buildVerificationResultItems(
  summary: VerificationSummary | null,
): Array<{ label: string; value: string }> {
  return [
...
export function VerificationOverviewGrid({
  summary,
}: {
  summary: VerificationSummary | null;
}): React.ReactElement {
  ...
  return (
    <>
      ...
      <div className="mt-4 rounded-md border px-3 py-2 text-[12px]" style={panelStyle('var(--bg)')}>
        ...
        <div className="mt-1 whitespace-pre-wrap" style={{ color: 'var(--text-muted)' }}>
          {summary?.summary ?? 'No verification run has been recorded yet.'}
        </div>
      </div>
    </>
  );
