...
export function ContextMetricsHeader({
  session,
}: {
...
export function ContextMetricsCards({
  session,
}: {
  session: TaskSessionRecord | null;
}): React.ReactElement {
  ...
  return (
    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <MetricCard
        ...
        value={String(packet?.omittedCandidates.length ?? 0)}
      />
...
export function ContextBudgetNotes({
  session,
}: {
  session: TaskSessionRecord | null;
...
function MetricCard({ label, value }: { label: string; value: string }): React.ReactElement {
  return (
    <div className="rounded-md border p-3" style={panelStyle('var(--bg)')}>
      ...
      <div className="mt-1 text-[18px] font-semibold" style={{ color: 'var(--text)' }}>
        {value}
      </div>
    </div>
  );
