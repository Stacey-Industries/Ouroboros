  ...
  return (
    <div className="rounded-lg border p-4" style={panelStyle()}>
      ...
      <div className="mt-3 space-y-3">
        {summary?.commandResults.length ? (
          summary.commandResults.map((result) => (
            <StepResultCard key={result.stepId} result={result} />
          ))
        ) : (
          <EmptyMessage message="No verification commands have been recorded yet." />
        )}
      </div>
    </div>
  );
...
function StepResultCard({
  ...
  result: NonNullable<VerificationSummary['commandResults']>[number];
}): React.ReactElement {
  ...
  return (
    <div className="rounded-md border p-3 text-[12px]" style={panelStyle('var(--bg)')}>
      ...
      <div className="mt-2 grid gap-3 md:grid-cols-2">
        <KeyValue
          label="Exit code"
          value={typeof result.exitCode === 'number' ? String(result.exitCode) : '—'}
        />
        <KeyValue
          label="Duration"
...
function CollapsibleOutput({
  ...
  return (
    <details className="mt-3">
      <summary className="cursor-pointer select-none" style={{ color: 'var(--text)' }}>
        {title}
      </summary>
      <pre
        className="mt-2 overflow-x-auto whitespace-pre-wrap rounded-md p-3 text-[11px]"
        style={{
...
export function VerificationIssuesCard({
  summary,
}: {
  summary: VerificationSummary | null;
}): React.ReactElement {
      ...
      <div className="mt-3 space-y-2">
        {summary?.issues.length ? (
          summary.issues.map((issue, index) => (
            <IssueCard key={`${issue.filePath ?? 'issue'}-${index}`} issue={issue} />
          ))
        ) : (
          <EmptyMessage message="No verification issues are attached to this session." />
        )}
      </div>
    </div>
  );
...
function IssueCard({
  ...
  issue: NonNullable<VerificationSummary['issues']>[number];
}): React.ReactElement {
  const tone = resolveStatusTone(
    issue.severity === 'error' ? 'failed' : issue.severity === 'warning' ? 'running' : 'pending',
  );
  return (
    <div className="rounded-md border px-3 py-2 text-[12px]" style={panelStyle('var(--bg)')}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        {issue.filePath ? (
          <span style={{ color: 'var(--text-muted)' }}>{issue.filePath.replace(/\\/g, '/')}</span>
        ) : null}
      </div>
...
function KeyValue({
  ...
  return (
    <div>
      ...
      <div
        className={`mt-1 ${breakAll ? 'break-all' : ''} ${multiline ? 'whitespace-pre-wrap' : ''}`}
...
function EmptyMessage({ message }: { message: string }): React.ReactElement {
  return (
    <div
      className="rounded-md border px-3 py-4 text-[12px]"
      style={{ ...panelStyle('var(--bg)'), color: 'var(--text-muted)' }}
    >
      {message}
    </div>
  );
