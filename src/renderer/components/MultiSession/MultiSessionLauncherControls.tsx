...
export function IconButton({ ariaLabel, children, defaultColor, hoverColor, onClick, title }: IconButtonProps): React.ReactElement {
  return (
    <button
      onClick={onClick}
      className="shrink-0 rounded p-1 transition-colors"
      style={{ color: defaultColor, background: 'transparent', border: 'none', cursor: 'pointer' }}
      onMouseEnter={(event) => { event.currentTarget.style.color = hoverColor; }}
      onMouseLeave={(event) => { event.currentTarget.style.color = defaultColor; }}
      title={title}
...
function updateAddButtonColors(button: HTMLButtonElement, canAddSlot: boolean, hover: boolean): void {
  button.style.borderColor = hover && canAddSlot ? 'var(--accent)' : 'var(--border)';
  button.style.color = hover && canAddSlot ? 'var(--text)' : canAddSlot ? 'var(--text-muted)' : 'var(--text-faint)';
}

export function AddSessionButton({ canAddSlot, onAddSlot }: { canAddSlot: boolean; onAddSlot: () => void; }): React.ReactElement {
  return (
    <button
      onClick={onAddSlot}
      disabled={!canAddSlot}
      className="flex items-center gap-1 rounded px-2.5 py-1.5 text-[11px] font-medium transition-colors"
      style={{ background: 'transparent', border: '1px solid var(--border)', color: canAddSlot ? 'var(--text-muted)' : 'var(--text-faint)', cursor: canAddSlot ? 'pointer' : 'not-allowed', fontFamily: 'var(--font-ui)', opacity: canAddSlot ? 1 : 0.5 }}
