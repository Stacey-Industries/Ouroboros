...
const STATUS_CONFIG = {
  ...
  running: {
    label: 'Running',
    dotColor: 'var(--interactive-accent)',
    bgTint: 'color-mix(in srgb, var(--interactive-accent) 5%, transparent)',
  },
  complete: {
    label: 'Done',
    dotColor: 'var(--status-success)',
    bgTint: 'color-mix(in srgb, var(--status-success) 5%, transparent)',
  },
  error: {
    label: 'Error',
    dotColor: 'var(--status-error)',
    bgTint: 'color-mix(in srgb, var(--status-error) 5%, transparent)',
  },
} as const;

function getToolCallColor(status: ToolCallEvent['status']): string {
  if (status === 'pending') return 'var(--interactive-accent)';
  return status === 'success' ? 'var(--status-success)' : 'var(--status-error)';
}

function updateBorderButtonColors(button: HTMLButtonElement, hover: boolean): void {
  button.style.borderColor = hover ? 'var(--interactive-accent)' : 'var(--border-default)';
  button.style.color = hover ? 'var(--interactive-accent)' : 'var(--text-faint)';
}

export const CompactToolCall = memo(function CompactToolCall({
  call,
}: {
  call: ToolCallEvent;
}): React.JSX.Element {
  return (
    <div className="flex items-center gap-1.5 truncate px-2 py-0.5" style={{ fontSize: '10px' }}>
      <span
        ...
        style={{ background: getToolCallColor(call.status) }}
      />
      <span className="shrink-0 font-medium text-text-semantic-muted">{call.toolName}</span>
      <span className="truncate text-text-semantic-faint">
        {call.status === 'pending' ? '... ' : ''}
        {call.input}
      </span>
    </div>
...
export function useElapsedLabel(startedAt: number, running: boolean): string {
...
function SessionHeader({
  ...
  return (
    <div className="flex flex-shrink-0 items-center gap-2 px-2.5 py-1.5 border-b border-border-semantic">
      ...
      />
      <span
        className="min-w-0 flex-1 truncate text-[11px] font-medium text-text-semantic-primary"
        title={session.taskLabel}
      >
        {session.taskLabel}
      </span>
      <span className="shrink-0 text-[10px] font-medium" style={{ color: status.dotColor }}>
        {status.label}
      </span>
  ...
  return (
    <div className="py-1">
      {latestCalls.map((call) => (
        <CompactToolCall key={call.id} call={call} />
      ))}
    </div>
  );
}
...
function ViewFullButton({ onClick }: { onClick: () => void }): React.JSX.Element {
  return (
    <button
      onClick={onClick}
...
function SessionFooter({
  ...
  return (
    <div className="flex flex-shrink-0 items-center gap-2 px-2.5 py-1 border-t border-border-semantic">
      ...
      <span className="font-mono text-[10px] text-interactive-accent">
        ~{formatCost(estimateSessionCost(session))}
      </span>
...
export function EmptySessionCell({ label }: { label: string }): React.JSX.Element {
  return (
    <div className="flex h-full flex-col items-center justify-center rounded bg-surface-panel border border-border-semantic">
      ...
      <span className="text-[10px] italic text-text-semantic-faint">
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${session.status === 'error' ? 'var(--status-error)' : 'var(--border-default)'}`,
      }}
    >
      <SessionHeader elapsed={elapsed} isRunning={isRunning ?? false} session={session} />
      <div className="min-h-0 flex-1 overflow-hidden">
        <ToolCallFeed isRunning={isRunning ?? false} latestCalls={session.toolCalls.slice(-3)} />
      </div>
