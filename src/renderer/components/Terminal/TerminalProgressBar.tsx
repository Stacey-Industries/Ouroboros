...
export function TerminalProgressBar({
  subscribe,
}: TerminalProgressBarProps): React.ReactElement | null {
  const [display, setDisplay] = useState<ProgressDisplay>({ visualState: 'hidden', value: 0 });
  const fadeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (!subscribe) return;
    const disposable = subscribe((progressState) =>
      handleProgressState(progressState, setDisplay, fadeTimerRef),
    );
    return () => {
      disposable.dispose();
      clearTimer(fadeTimerRef);
  ...
  return (
    ...
    >
      <div
        className={
          display.visualState === 'indeterminate'
...
export function TerminalProgressDot({
  ...
  useEffect(() => {
    if (!subscribe) return;
    const disposable = subscribe((progressState) =>
      handleProgressDotState(progressState, setActive, setIsError, fadeTimerRef),
    );
    return () => {
      disposable.dispose();
      clearTimer(fadeTimerRef);
