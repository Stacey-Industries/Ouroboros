...
function handleTabInKey(context: TerminalSetupLifecycleContext, domEvent: KeyboardEvent): boolean {
...
function moveCompletionSelection(
  ...
  const nextIndex =
    delta > 0 ? Math.min(currentIndex + 1, maxIndex) : Math.max(currentIndex - 1, 0);
  context.completionState.completionIndexRef.current = nextIndex;
  context.completionState.setCompletionIndex(nextIndex);
  return true;
}
...
function applySelectedCompletion(context: TerminalSetupLifecycleContext): void {
  const selected =
    context.completionState.completionsRef.current[
...
function resetHistoryPos(context: TerminalSetupLifecycleContext, code: string): void {
  if (!['ArrowLeft', 'ArrowRight', 'ShiftLeft', 'ShiftRight'].includes(code)) {
    context.historyRefs.histPosRef.current = -1;
  }
}

function handleEnter(context: TerminalSetupLifecycleContext, key: string): boolean {
  if (key !== '\r' && key !== '\n') return false;
...
function handleBackspaceKey(context: TerminalSetupLifecycleContext, key: string): boolean {
...
function trackPrintableKey(context: TerminalSetupLifecycleContext, key: string): void {
