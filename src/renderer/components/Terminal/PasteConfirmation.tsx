...
export function PasteConfirmBanner({ text, onConfirm, onConfirmSingleLine, onCancel }: PasteConfirmBannerProps): React.ReactElement {
  ...
  return (
    <div className="bg-surface-panel text-text-semantic-primary border-t border-border-semantic" style={bannerStyle}>
      <span className="text-text-semantic-muted" style={{ flex: 1 }}>
        Paste {text.length.toLocaleString()} characters{formatLineCount(text)}?
      </span>
      <button onClick={onConfirm} autoFocus className="text-text-semantic-on-accent" style={confirmBtnStyle}>Paste</button>
      {multiline && (
        <button onClick={onConfirmSingleLine} className="text-interactive-accent" style={singleLineBtnStyle}>
          Single line
        </button>
      )}
      <button onClick={onCancel} className="border border-border-semantic text-text-semantic-muted" style={cancelBtnStyle}>Cancel</button>
    </div>
  )
}
