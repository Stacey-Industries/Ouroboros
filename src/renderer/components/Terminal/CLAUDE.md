...
| `TerminalTabs.tsx` | Tab strip with rename, close, reorder (DnD in `TerminalTabs.dnd.ts`) |
...
- **Session key for `useTerminalSetup`**: the `useEffect` depends only on `sessionId`. Changing any other prop does not re-bootstrap — update effect deps deliberately.
