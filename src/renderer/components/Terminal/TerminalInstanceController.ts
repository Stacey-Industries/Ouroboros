...
function useTerminalConfig(): {
  ...
  useEffect(() => { let cancelled = false; void Promise.all([window.electronAPI.config.get('terminalFontSize'), window.electronAPI.config.get('terminalCursorStyle')]).then(([fs, cs]) => { if (cancelled) return; if (typeof fs === 'number' && fs >= 8 && fs <= 32) setFontSize(fs); if (cs === 'block' || cs === 'underline' || cs === 'bar') setCursorStyle(cs); setLoaded(true) }).catch((error) => { console.error('[terminal] Failed to load terminal config:', error); if (!cancelled) setLoaded(true) }); return () => { cancelled = true } }, [])
...
export function useTerminalInstanceController(
  ...
  const { restoreSession } = useTerminalPersistence({ sessionId: foundation.sessionId, terminalRef: foundation.terminalRef, serializeAddonRef: foundation.serializeAddonRef, projectRoot, cwd: foundation.cwd })
  ...
  const cmdSearchActions = useCommandSearchActions(foundation.sessionId, historyState.historyHook.cmdSearch, foundation.terminalRef)
  useTerminalSetupBridge({ foundation, historyState, setPendingPaste: pasteState.setPendingPaste, setRichInputActive: richInputState.setRichInputActive, setSelectionTooltip: tooltipState.setSelectionTooltip, initialFontSize: config.fontSize, initialCursorStyle: config.cursorStyle })
