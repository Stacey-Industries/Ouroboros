...
function CommandItem({ cmd, isSelected, onSelect, onHover }: {
  ...
  return (
    <div
      ...
      className={isSelected ? 'text-text-semantic-primary bg-surface-raised' : 'text-text-semantic-muted'}
      style={{
  ...
  return (
    ...
    >
      <div className="border-b border-border-semantic" style={inputRowStyle}>
        <span className="text-interactive-accent" style={{ fontSize: 11, flexShrink: 0 }}>bck-i-search:</span>
        <input ref={inputRef} type="text" value={query} onChange={(e) => setQuery(e.target.value)} className="bg-surface-base text-text-semantic-primary border border-border-semantic rounded" style={inputStyle} placeholder="Type to filter history..." />
        <button onClick={onClose} className="text-text-semantic-muted" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', fontSize: 14 }} title="Close (Esc)">&#x2715;</button>
      </div>
      <div style={{ overflowY: 'auto', flex: 1 }}>
        {filtered.length === 0 && <div className="text-text-semantic-muted" style={{ padding: '8px 12px' }}>No matching commands</div>}
      ...
      <div className="text-text-semantic-muted border-t border-border-semantic" style={{ padding: '4px 10px', fontSize: 10, flexShrink: 0 }}>
