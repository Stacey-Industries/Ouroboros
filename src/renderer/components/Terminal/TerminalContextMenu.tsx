...
function MenuItem({ item }: { item: MenuItemDef }): React.ReactElement {
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (!item.disabled)
          e.currentTarget.style.backgroundColor = 'rgba(var(--accent-rgb, 88, 166, 255), 0.15)';
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      {item.shortcut && (
        <span
          ...
          style={{ fontSize: '0.6875rem', fontFamily: 'var(--font-ui)' }}
        >
          {item.shortcut}
        </span>
      )}
...
export function TerminalContextMenu({
  ...
  return (
    <div
      ...
      style={{ position: 'fixed', left: state.x, top: state.y, zIndex: 9999, ...menuStyle }}
    >
      {items.map((item) => (
