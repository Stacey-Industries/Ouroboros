...
export interface Osc133State {
  ...
  pendingQueueRef: React.MutableRefObject<Array<{
    sequence: string
...
export function parseAndStripOsc133(
  raw: string,
  state: Osc133State,
): string {
...
function onPromptStart(absRow: number, state: Osc133State): void {
  state.currentBlockRef.current = {
    promptRow: absRow, outputRow: null, exitCode: -1, complete: false,
  }
...
function registerBlockDecoration(
  ...
  try {
    ...
    const height = Math.min(
      Math.max(1, absCursor - block.promptRow + 1),
      term.rows * 3,
    )
    const marker = term.registerMarker(offset)
    if (!marker) return
    const dec = term.registerDecoration({
      marker, x: 0, width: term.cols, height, layer: 'bottom',
    })
    if (!dec) return
    dec.onRender((el) => {
      el.style.cssText = [
