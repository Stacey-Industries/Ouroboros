...
function useSearchState(searchAddon: SearchAddon) {
  ...
  const [matchInfo, setMatchInfo] = useState<{ resultIndex: number; resultCount: number } | null>(
    null,
  );
...
function TerminalSearchBarView({
  ...
  return (
    ...
    >
      ...
      />
      {matchLabel && (
        <span className="text-text-semantic-muted" style={{ minWidth: 60, textAlign: 'center' }}>
          {matchLabel}
        </span>
      )}
      <SearchNavButtons
