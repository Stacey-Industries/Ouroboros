...
function useCommandBlockState(): CommandBlockState {
  ...
  const refs = useRef<CommandBlockRefs>({
    blocks: [],
    currentBlock: null,
    heuristicTimer: null,
    osc133Active: null,
...
function scheduleHeuristicCheck(
  state: CommandBlockState,
  row: number,
  term: Terminal,
  customPattern: RegExp | null,
 ...
 */
function handleOsc633Event(event: ShellIntegrationEvent, state: CommandBlockState): void {
  switch (event.type) {
    case 'promptStart':
...
export function useCommandBlocksController(
  ...
  return {
    ...
    getBlockOutput,
    reset,
    osc133Active: state.osc133Active,
  };
}
