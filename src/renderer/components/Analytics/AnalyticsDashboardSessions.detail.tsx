  ...
  return (
    <div className="flex items-center gap-2">
      <span
        ...
        style={{ background: getSessionStatusColor(status) }}
      />
      ...
      {model ? (
        <span
          className="text-[9px] px-1.5 py-0.5 rounded font-semibold text-interactive-accent"
          style={{ background: 'color-mix(in srgb, var(--interactive-accent) 15%, transparent)' }}
        >
...
const SessionTokenBreakdown = memo(function SessionTokenBreakdown({
  ...
  return (
    ...
    >
      <div className="text-[9px] font-medium uppercase tracking-wider mb-1 text-text-semantic-faint">
        Token Breakdown
      </div>
      <div
      ...
      >
        <span className="text-text-semantic-muted">
          In: <span className="text-text-semantic-primary">{formatTokens(inputTokens)}</span>
        </span>
        <span className="text-text-semantic-muted">
          Out: <span className="text-text-semantic-primary">{formatTokens(outputTokens)}</span>
        </span>
        <span className="text-text-semantic-muted">
          Total: <span className="text-interactive-accent">{formatTokens(totalTokens)}</span>
        </span>
      </div>
    </div>
  );
...
const FileEditRow = memo(function FileEditRow({
  ...
  return (
    <div className="flex items-center justify-between text-[10px]">
      <span
        className="truncate text-text-semantic-muted"
        style={{ fontFamily: 'var(--font-mono)' }}
        title={filePath}
      >
      ...
      <span
        ...
        style={{
          ...
          color: count >= 3 ? 'var(--status-error)' : 'var(--text-faint)',
        }}
  ...
  return (
    <div className="flex items-center gap-1.5 text-[10px]">
      <span
        ...
        style={{ background: getToolCallStatusColor(toolCall.status) }}
      />
      <span
        className="font-semibold flex-shrink-0"
        style={{ color: getToolColor(toolCall.toolName), width: '48px' }}
      >
        {toolCall.toolName}
      </span>
      <span
        className="truncate flex-1 min-w-0 text-text-semantic-muted"
        style={{ fontFamily: 'var(--font-mono)' }}
        title={toolCall.input}
      >
        {toolCall.input}
      </span>
      {toolCall.duration !== undefined ? (
        <span
          className="tabular-nums flex-shrink-0 text-text-semantic-faint"
          style={{ fontFamily: 'var(--font-mono)' }}
        >
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 overflow-y-auto" style={{ maxHeight: '120px' }}>
        {session.toolCalls.length === 0 ? (
          <span className="text-[10px] italic text-text-semantic-faint">No tool calls</span>
        ) : (
          session.toolCalls.map((toolCall) => (
            <SessionToolCallRow key={toolCall.id} toolCall={toolCall} />
          ))
        )}
      </div>
    </div>
...
const SessionErrorRow = memo(function SessionErrorRow({
  ...
  return (
    <div
      ...
      title={toolCall.output ?? toolCall.input}
    >
      <span style={{ color: getToolColor(toolCall.toolName) }}>{toolCall.toolName}</span>:{' '}
...
const SessionErrorsSection = memo(function SessionErrorsSection({
  ...
  return (
    <div
      className="rounded-md p-2 mt-2 bg-surface-base"
      style={{ border: '1px solid var(--status-error)' }}
    >
      <div className="text-[9px] font-medium uppercase tracking-wider mb-1 text-status-error">
        Errors ({session.errorCount})
      </div>
      <div className="flex flex-col gap-0.5 overflow-y-auto" style={{ maxHeight: '60px' }}>
        {errorCalls.map((toolCall) => (
          <SessionErrorRow key={toolCall.id} toolCall={toolCall} />
        ))}
      </div>
    </div>
  );
...
export const SessionDetailPanel = memo(function SessionDetailPanel({
  ...
  return (
    <div className="px-4 py-3 border-b border-border-semantic bg-surface-raised">
      ...
      />
      <div className="text-[10px] mb-3 truncate text-text-semantic-muted" title={session.taskLabel}>
        {session.taskLabel}
      </div>
