...
function getExtensionStoreApi(): ExtensionStoreApi | undefined {
...
function useExtensionStoreSearchState({ sourceRef, setError }: StoreStateArgs) {
  ...
  const loadMore = useCallback(() => {
    ...
    if (nextOffset < totalSize)
      executeSearch(queryRef.current, categoryRef.current, nextOffset, true);
  ...
  const setQuery = useCallback(
    (nextQuery: string) => {
      ...
      debounceRef.current = setTimeout(
        () => executeSearch(nextQuery, categoryRef.current),
        SEARCH_DEBOUNCE_MS,
      );
    },
    [executeSearch],
...
export function useExtensionStoreModel(): ExtensionStoreModel {
  ...
  const {
    ...
    setCategoryFilter,
    resetResults,
  } = useExtensionStoreSearchState({ sourceRef, setError });
  const { selectedExtension, selectExtension, clearSelection } = useExtensionStoreSelectionState({
    sourceRef,
    setError,
  });
  const {
    ...
    toggleEnabled,
    refreshInstalled,
  } = useExtensionStoreInventoryState({ sourceRef, setError });
