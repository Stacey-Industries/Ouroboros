...
function applyComposerSuccess(args: SendMessageArgs, result: Awaited<ReturnType<typeof sendAgentChatRequest>>): void { args.setAttachments?.([]); mergeReturnedThread(result.thread, args.setThreads, args.setActiveThreadId); /* pendingUserMessage stays set until the persisted user message appears in the thread — see usePendingUserMessageClearEffect. Clearing here would hide the user's bubble for the gap between 'isSending → false' and the thread_snapshot landing. */ clearPersistedDraft(result.thread?.id ?? args.activeThreadId); if (isDraftThreadId(args.activeThreadId) && result.thread) clearPersistedDraft(args.activeThreadId); }
