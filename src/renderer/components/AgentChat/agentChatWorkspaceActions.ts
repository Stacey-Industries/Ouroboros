...
type BuildWorkspaceModelArgs = AgentChatActionState & {
  ...
  setMentionRanges: (ranges: import('../../../shared/types/orchestrationDomain').UserSelectedFileRange[]) => void;
...
export function useSendMessageAction(args: SendMessageArgs): () => Promise<void> {
  ...
  return useCallback(async () => { await sendComposerMessage(argsRef.current); }, []);
}
export function useOpenLinkedDetailsAction(
  ...
  return useCallback(async (link?: AgentChatOrchestrationLink): Promise<void> => {
    ...
    } catch (detailsError) { setError(getErrorMessage(detailsError)); }
  }, [setError]);
}
...
export function useEditAndResendAction(args: AgentChatActionArgs): (message: AgentChatMessageRecord) => Promise<void> {
  const argsRef = useRef(args);
  argsRef.current = args;
  return useCallback(async (message: AgentChatMessageRecord): Promise<void> => { await editAndResendOnBranch(argsRef.current, message); }, []);
}
export function useRetryMessageAction(args: AgentChatActionArgs): (message: AgentChatMessageRecord) => Promise<void> {
  const argsRef = useRef(args);
  argsRef.current = args;
  return useCallback(async (message: AgentChatMessageRecord): Promise<void> => { await sendResentMessage(argsRef.current, message, 'retry'); }, []);
}
export function useBranchFromMessageAction(
  ...
  return useCallback(async (message: AgentChatMessageRecord): Promise<void> => {
    ...
    } catch (branchError) { setError(getErrorMessage(branchError)); }
  }, [setActiveThreadId, setError, setThreads]);
}
...
export function useRevertMessageAction(
  ...
  return useCallback(async (message: AgentChatMessageRecord): Promise<void> => {
    ...
    if (!message.orchestration?.preSnapshotHash)
      return void setError('No snapshot was captured before this agent turn. Revert is unavailable.');
    try {
    ...
    } catch (revertError) { setError(getErrorMessage(revertError)); }
  }, [setError, setThreads]);
}
export function useAgentChatActions(args: AgentChatActionArgs): AgentChatActionState {
  ...
  return { branchFromMessage, deleteThread, editAndResend, openLinkedDetails, retryMessage, revertMessage, selectThread: selectionActions.selectThread, sendMessage, startNewChat: selectionActions.startNewChat, stopTask };
}
