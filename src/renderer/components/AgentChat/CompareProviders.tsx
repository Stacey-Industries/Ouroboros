...
interface ContentProps {
  ...
  providerIdA: string;
  providerIdB: string;
  setProviderIdA: (v: string) => void;
  setProviderIdB: (v: string) => void;
...
function buildPanesStyle(isMobile: boolean): React.CSSProperties {
  if (isMobile) return { display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, gap: '8px', padding: '8px', overflow: 'hidden' };
  return { display: 'flex', flex: 1, minHeight: 0, gap: '8px', padding: '8px', overflow: 'hidden' };
}

function ShowDiffFooter({ onShow }: { onShow: () => void }): React.ReactElement {
  return (
    <div style={{ padding: '8px 12px', borderTop: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'flex-end', flexShrink: 0 }}>
      <button onClick={onShow} style={{ padding: '5px 14px', borderRadius: '4px', border: 'none', background: 'var(--interactive-accent)', color: 'var(--text-on-accent)', fontSize: '13px', cursor: 'pointer' }}>
        Show diff
      </button>
    </div>
  );
...
function ComparePanes(p: ContentProps & { labelA: string; labelB: string }): React.ReactElement {
  const panesStyle = buildPanesStyle(p.isMobile);
  if (p.showDiff) return <CompareProvidersDiff textA={p.state.paneA.text} textB={p.state.paneB.text} labelA={p.labelA} labelB={p.labelB} />;
  return (
    <div style={panesStyle}>
      <CompareProvidersOutputPane {...p.state.paneA} label={p.labelA} />
      <CompareProvidersOutputPane {...p.state.paneB} label={p.labelB} />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <CompareProvidersHeader
        ...
        providers={PROVIDER_OPTIONS} isRunning={p.isRunning} onRun={p.onRun} onCancel={p.onCancel}
      />
      <ComparePanes {...p} labelA={labelA} labelB={labelB} />
      {bothDone && !p.showDiff && <ShowDiffFooter onShow={() => p.setShowDiff(true)} />}
    </div>
  );
}
...
function useCompareProvidersState(projectPath: string, start: ReturnType<typeof useCompareSession>['start'], cancel: ReturnType<typeof useCompareSession>['cancel']) {
  ...
  return { prompt, setPrompt, providerIdA, providerIdB, setProviderIdA, setProviderIdB, warningDismissed, setWarningDismissed, showDiff, setShowDiff, onRun, onCancel };
}
...
export function CompareProviders({ isOpen, onClose, projectPath }: CompareProvidersProps): React.ReactElement | null {
  ...
  const s = useCompareProvidersState(projectPath, start, cancel);
  if (!isOpen) return null;
  const isMobile = viewport === 'phone';
  const isRunning = state.status === 'running' || state.status === 'starting';
  const contentProps: ContentProps = {
    state, prompt: s.prompt, setPrompt: s.setPrompt,
    providerIdA: s.providerIdA, providerIdB: s.providerIdB,
    setProviderIdA: s.setProviderIdA, setProviderIdB: s.setProviderIdB,
    warningDismissed: s.warningDismissed, onDismissWarning: () => s.setWarningDismissed(true),
    showDiff: s.showDiff, setShowDiff: s.setShowDiff, isRunning, onRun: s.onRun, onCancel: s.onCancel, isMobile,
  };
  if (isMobile) {
    return (
  ...
  return (
    <div style={OVERLAY_STYLE} onClick={onClose}>
      <div style={MODAL_STYLE} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-label="Compare providers">
        <CompareContent {...contentProps} />
      </div>
    </div>
  );
