...
- **Lexical composer (Wave 81)**: `LexicalChatComposer` is the only composer engine. `BeautifulMentionsPlugin` owns the `@`-dropdown via `LexicalMentionMenuItem`; the legacy `MentionAutocomplete` component is no longer rendered (its types — `MentionItem` / `MentionType` / `SymbolGraphNode` — are still exported and used everywhere). `showMentionsOnDelete={true}` is load-bearing: backspacing a mention chip replaces the `BeautifulMentionNode` with a `@` text node, which fires the `'destroyed'` mutation that `LexicalMentionBridge` translates into `removeMention(key)` on the chip-bar store. Without it, default Lexical decorator-delete doesn't reliably surface the removal. The bridge keeps a `nodeKey → mentionItem.key` cache so it can fire `removeMention` even after the node is gone, plus an `editor.registerUpdateListener` reconciler as a safety net for any missed mutation events.
- **Lexical mention chip styling lives in `INITIAL_CONFIG.theme.beautifulMentions['@']`** in `LexicalChatComposer.tsx` — the library renders the chip as a plain `<span>` with no styling unless a theme is provided, which produces invisible text on most surfaces. Don't drop the theme entry.
- **`LexicalMentionMenuItem` strips `option.data` props** before forwarding to `<li>` (`mentionKey`, `mentionType`, `mentionLabel`, `mentionPath`, `estimatedTokens`, `startLine`, `endLine`, `symbolType`, `value`, `itemValue`, `label`). `BeautifulMentionsPlugin` spreads them into menu-item props alongside legitimate DOM attributes (role, tabIndex, aria-*, onMouseDown), so blind `{...rest}` triggers React unknown-prop warnings.
- **`LexicalMentionMenu` flips above the cursor**: Lexical's default `LexicalTypeaheadMenuPlugin` positions the menu *below* the cursor and only auto-flips above when there's room within the contenteditable root — which the chat composer rarely has. The custom `menuComponent` uses `absolute bottom-full` to flip unconditionally.
- **Slash-command keyboard-Enter path needs `slashCommandContext` + `onAddMention` plumbed**: `slashKeyboardNav.useSlashEnter` previously dropped them, so non-`/clear` commands silently no-op'd via Enter while mouse-click selection (which goes through `useSlashSelectHandler`) worked. Both paths now pass them through `SlashCommandPlugin` props.
- **`/spec featureName` Enter-send flow is structurally broken (pre-existing)**: `extractSlashQuery` closes the menu on whitespace, so the menu is closed by the time the user hits Enter on `/spec myFeature`. Tracked in `roadmap/follow-ups/follow-ups.md` — fix path is to add a send-time interceptor mirroring `useResearchIntercept`.
