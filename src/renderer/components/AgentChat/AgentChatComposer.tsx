...
const noop = (): void => {};
const DIFF_MENTION: MentionItem = {
  type: 'diff',
  key: '@diff',
  label: 'Git Diff',
  path: '@diff',
...
function renderMentionChips(
  ...
  return useMentionSystem ? (
    <MentionChipsBar
      mentions={mentions}
      onRemove={onRemoveMention ?? noop}
      totalTokens={mentions.reduce((sum, mention) => sum + mention.estimatedTokens, 0)}
    />
  ) : null;
}
...
function removeTriggerBeforeCursor(
  textareaRef: React.RefObject<HTMLTextAreaElement>,
  lastSyncedDraft: React.MutableRefObject<string>,
  onChange: (value: string) => void,
): void {
...
function useImageAttachmentHandlers(
  attachments: ImageAttachment[],
  onAttachmentsChange?: (attachments: ImageAttachment[]) => void,
) {
...
function handleComposerShortcutKeyDown(args: {
  event: React.KeyboardEvent<HTMLTextAreaElement>;
  chatOverrides?: ChatOverrides;
  onChatOverridesChange?: (overrides: ChatOverrides) => void;
  defaultProvider?: 'claude-code' | 'codex' | 'anthropic-api';
...
function handleComposerKeyDown(args: {
  event: React.KeyboardEvent<HTMLTextAreaElement>;
...
function useComposerDraftHandlers(args: {
  textareaRef: React.RefObject<HTMLTextAreaElement>;
  lastSyncedDraft: React.MutableRefObject<string>;
  draft: string;
  messages?: AgentChatMessageRecord[];
...
function useComposerAutocompleteReset(
  setSelectedIndex: React.Dispatch<React.SetStateAction<number>>,
    ...
    >
      <div className="px-3">
        ...
        <ComposerMenus
          ...
          useMentionSystem={useMentionSystem}
        />
        <ComposerInput
          canSend={canSend}
          disabled={disabled}
