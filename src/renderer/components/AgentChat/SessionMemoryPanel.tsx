...
function sortMemories(memories: SessionMemoryEntry[]): SessionMemoryEntry[] {
  return [...memories].sort((a, b) => (b.confidence !== a.confidence ? b.confidence - a.confidence : new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()));
}

function MemoryEmptyState(): React.ReactElement {
  return (
    <div className="flex flex-col items-center justify-center gap-2 p-8 text-center text-text-semantic-muted" style={{ fontSize: 12, fontFamily: 'var(--font-ui)' }}>
      ...
      <span className="text-[11px] text-text-semantic-faint">Memories are created automatically from chat sessions, or manually with /remember.</span>
    </div>
  );
}
...
function MemoryPanelHeader({ count }: { count: number }): React.ReactElement {
  return (
    <div className="flex items-center justify-between border-b px-3 py-2" style={{ borderColor: 'var(--border)', fontFamily: 'var(--font-ui)' }}>
      <span className="text-[11px] font-semibold uppercase tracking-wider text-text-semantic-muted" style={{ letterSpacing: '0.04em' }}>Session Memory</span>
      {count > 0 && <span className="rounded-full bg-interactive-accent px-1.5 py-0.5 text-[10px] font-semibold text-white" style={{ minWidth: 18, textAlign: 'center' }}>{count}</span>}
    </div>
  );
}

async function loadWorkspaceMemories(workspaceRoot: string, setMemories: React.Dispatch<React.SetStateAction<SessionMemoryEntry[]>>, mountedRef: React.MutableRefObject<boolean>, setLoading: React.Dispatch<React.SetStateAction<boolean>>): Promise<void> {
  try {
...
async function updateWorkspaceMemory(args: {
  ...
  try {
    ...
    if (result.success && result.memory) {
      setMemories((prev) => sortMemories(prev.map((entry) => (entry.id === id ? result.memory! : entry))));
    }
  } catch (err) {
...
function useSessionMemoryPanelModel(workspaceRoot: string | null): {
  ...
  handleUpdate: (id: string, updates: { content?: string; type?: string; relevantFiles?: string[] }) => void;
...
export const SessionMemoryPanel = memo(function SessionMemoryPanel({ workspaceRoot }: SessionMemoryPanelProps): React.ReactElement {
  const { loading, memories, handleUpdate, handleDelete } = useSessionMemoryPanelModel(workspaceRoot);

  if (loading) {
    return (
      <div className="flex h-full flex-col">
        <MemoryPanelHeader count={0} />
        <div className="flex flex-1 items-center justify-center text-xs text-text-semantic-muted">Loading memories...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
        {memories.length === 0 ? <MemoryEmptyState /> : memories.map((entry) => <SessionMemoryRow key={entry.id} entry={entry} onUpdate={handleUpdate} onDelete={handleDelete} />)}
      </div>
    </div>
  );
});
