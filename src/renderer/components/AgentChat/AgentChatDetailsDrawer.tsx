...
function DrawerSection(props: {
  ...
  return (
    <section className="rounded border border-[var(--border)] bg-[var(--bg)] px-3 py-3">
      <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">{props.title}</div>
    ...
    <div className="grid grid-cols-2 gap-x-3 gap-y-2 text-xs">
      {props.rows.filter((row) => row.value).map((row) => (
        <div key={row.label} className="min-w-0">
          <div className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">{row.label}</div>
          <div className="mt-1 truncate text-[var(--text)]" title={row.value ?? undefined}>{row.value}</div>
        </div>
      ))}
    </div>
  );
...
function DrawerTextBlock({ children }: { children: React.ReactNode }): React.ReactElement {
  return <div className="mt-3 text-xs leading-5 text-[var(--text-muted)]">{children}</div>;
}

function LoadingState(): React.ReactElement {
  return <div className="text-xs text-[var(--text-muted)]">Loading linked task details…</div>;
}

function ErrorState({ error }: { error: string }): React.ReactElement {
  return (
    <div className="rounded border border-[var(--border)] bg-[rgba(248,81,73,0.08)] px-3 py-3 text-xs leading-5 text-[var(--error,#f85149)]">
      {error}
    </div>
  );
}
...
function EmptyState(): React.ReactElement {
  return <div className="text-xs text-[var(--text-muted)]">No linked task details are available for this message yet.</div>;
}
...
function ContextSection({ details }: { details: AgentChatLinkedDetailsResult }): React.ReactElement | null {
  ...
  return (
    <DrawerSection title="Context">
      <div className="text-xs leading-5 text-[var(--text-muted)]">{budgetText}</div>
      {contextPacket.files.length > 0 ? (
        <div className="mt-3 space-y-2">
          {contextPacket.files.slice(0, 5).map((file) => (
            <div key={file.filePath} className="rounded border border-[var(--border)] px-2.5 py-2 text-xs">
              <div className="truncate text-[var(--text)]" title={file.filePath}>{file.filePath}</div>
...
function VerificationSection({ details }: { details: AgentChatLinkedDetailsResult }): React.ReactElement | null {
  ...
  return (
    <DrawerSection title="Verification">
      <div className="text-xs text-[var(--text)]">{`${verification.profile} • ${verification.status}`}</div>
      <div className="mt-2 text-xs leading-5 text-[var(--text-muted)]">{verification.summary || 'No verification summary available.'}</div>
      {verification.commandResults.length > 0 ? (
        <div className="mt-3 space-y-2">
          {verification.commandResults.slice(0, 4).map((result) => (
            <div key={result.stepId} className="rounded border border-[var(--border)] px-2.5 py-2 text-xs">
              <div className="flex items-center justify-between gap-2 text-[var(--text)]">
                <span className="truncate">{result.stepId}</span>
                <span className="text-[var(--text-muted)]">{result.status}</span>
              </div>
              {result.stderr?.trim() ? <div className="mt-1 text-[11px] text-[var(--error,#f85149)]">{result.stderr.trim()}</div> : null}
            </div>
          ))}
        </div>
      ) : null}
  ...
  return (
    <div className="mt-3 space-y-2">
      {issues.slice(0, 6).map((issue) => (
        <div key={issue} className="rounded border border-[var(--border)] px-2.5 py-2 text-xs text-[var(--text-muted)]">{issue}</div>
      ))}
    </div>
  );
}
  ...
  return (
    <div className="flex items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-3">
      <div className="min-w-0 flex-1">
        <div className="text-sm font-medium text-[var(--text)]">Task details</div>
        <div className="mt-1 text-xs text-[var(--text-muted)]">Inspect the linked orchestration task without leaving chat.</div>
      </div>
      <button
        onClick={onClose}
  ...
  return (
    <div className="flex items-center justify-between gap-2 border-b border-[var(--border)] px-4 py-2">
      ...
      <button
        ...
        className="rounded border border-[var(--border)] px-2 py-1 text-[11px] text-[var(--accent)] transition-colors duration-100 hover:opacity-80"
      >
        Open orchestration
      </button>
    </div>
...
function DrawerPanel(props: AgentChatDetailsDrawerProps): React.ReactElement {
  return (
    <div
      className={`flex h-full w-full max-w-[360px] flex-col border-l border-[var(--border)] bg-[var(--bg-secondary)] shadow-2xl transition-transform duration-200 ${props.isOpen ? 'translate-x-0' : 'translate-x-full'}`}
    >
