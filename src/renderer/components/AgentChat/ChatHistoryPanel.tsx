...
const SECTION_LABELS: Record<'active' | 'recent' | 'older', string> = {
  active: 'Active',
  recent: 'Today',
  older: 'Earlier',
};
...
export function ChatHistoryPanel({
  ...
  const sections = useMemo(() => {
    const groups: Record<'active' | 'recent' | 'older', AgentChatThreadRecord[]> = {
      active: [],
      recent: [],
      older: [],
    };
    for (const thread of filteredThreads) groups[classifyThread(thread)].push(thread);
    (Object.keys(groups) as Array<'active' | 'recent' | 'older'>).forEach((key) =>
