...
function SnippetView({
  ...
  return (
    <pre
      className="text-xs font-mono text-text-semantic-primary"
    ...
    >
      {snippet.map((line, i) => {
        ...
        return (
          ...
          >
            <span
              className="text-text-semantic-faint select-none"
...
function useFetchSnippet(resolvedPath: string, targetLine?: number): SnippetState {
  ...
  useEffect(() => {
    ...
    if (!api?.files?.readFile) {
      setError('File API unavailable');
      setLoading(false);
      return;
    }
...
function CardHeader({ raw, onOpen }: { raw: string; onOpen: () => void }): React.ReactElement {
  return (
    <div className="flex items-center justify-between px-3 py-1.5 border-b border-border-subtle">
      <span className="text-xs font-mono text-text-semantic-muted truncate" style={{ maxWidth: CARD_WIDTH - 100 }}>
        {raw}
      </span>
      <button type="button" onClick={onOpen} className="text-xs text-interactive-accent hover:text-interactive-hover ml-2 shrink-0">
        Open in editor
      </button>
    </div>
  );
...
function CardBody({ loading, error, snippet, startLine, targetLine }: { loading: boolean; error: string | null; snippet: string | null; startLine: number; targetLine: number | undefined }): React.ReactElement {
  return (
    <div className="overflow-auto" style={{ maxHeight: 240 }}>
      ...
      {!loading && snippet && <SnippetView snippet={snippet} startLine={startLine} targetLine={targetLine} />}
    </div>
  );
}
...
export function CitationHoverCard({
  ...
  useEffect(() => { if (anchorRef.current) setPos(computePosition(anchorRef.current)); }, [anchorRef]);
  const { snippet, startLine, error, loading } = useFetchSnippet(resolvedPath, fileRef.line);
  useDismissHandlers(cardRef, onClose);
  const handleOpenInEditor = useCallback(() => {
    window.dispatchEvent(new CustomEvent(OPEN_FILE_EVENT, { detail: { filePath: resolvedPath, line: fileRef.line, col: fileRef.col } }));
    onClose?.();
  }, [resolvedPath, fileRef.line, fileRef.col, onClose]);
  const card = (
    <div ref={cardRef} data-testid="citation-hover-card" className="fixed z-50 rounded border bg-surface-panel border-border-semantic shadow-lg" style={{ top: pos.top, left: pos.left, width: CARD_WIDTH, maxHeight: 300 }}>
      <CardHeader raw={fileRef.raw} onOpen={handleOpenInEditor} />
      <CardBody loading={loading} error={error} snippet={snippet} startLine={startLine} targetLine={fileRef.line} />
    </div>
  );
