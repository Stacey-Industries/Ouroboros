...
export function AgentChatDiffReview({
  ...
  const [fileStatuses, setFileStatuses] = useState<Record<string, FileStatus>>(() =>
    Object.fromEntries(files.map((file) => [file.path, 'pending' as FileStatus])),
  );
  ...
  const handleAcceptAll = useCallback(() => {
    setFileStatuses(Object.fromEntries(files.map((file) => [file.path, 'accepted' as FileStatus])));
    onAcceptAll?.();
  }, [files, onAcceptAll]);
  const handleRejectAll = useCallback(() => {
    setFileStatuses(Object.fromEntries(files.map((file) => [file.path, 'rejected' as FileStatus])));
    onRejectAll?.();
  }, [files, onRejectAll]);
