...
const TOOL_CATEGORIES = [
  {
    category: 'read',
    label: (count: number) => `Read ${count} file${count === 1 ? '' : 's'}`,
    matches: new Set(['Read', 'read_file']),
  },
  {
    category: 'edit',
    label: (count: number) => `Edited ${count} file${count === 1 ? '' : 's'}`,
    matches: new Set([
      'Edit',
      'edit_file',
      'MultiEdit',
  ...
  {
    category: 'search',
    label: (count: number) => `${count} search${count === 1 ? '' : 'es'}`,
    matches: new Set(['Grep', 'search_files', 'Glob', 'find_files']),
  },
  {
    category: 'bash',
    label: (count: number) => `${count} command${count === 1 ? '' : 's'}`,
    matches: new Set(['Bash', 'execute_command']),
  },
] as const;

function summarizeToolTypes(blocks: Array<AgentChatContentBlock & { kind: 'tool_use' }>): string {
  const counts = new Map<string, number>([
    ...TOOL_CATEGORIES.map((entry): [string, number] => [entry.category, 0]),
    ['other', 0],
  ]);
  for (const block of blocks) {
    const category =
      TOOL_CATEGORIES.find((entry) => entry.matches.has(block.tool))?.category ?? 'other';
  ...
  return TOOL_CATEGORIES.map((entry) => {
    ...
    .concat(
      (counts.get('other') ?? 0)
        ? `${counts.get('other')} other tool${(counts.get('other') ?? 0) === 1 ? '' : 's'}`
        : null,
    )
    .filter(Boolean)
    .join(', ');
 ...
 */
function ToolGroupCards({ blocks }: { blocks: Array<AgentChatContentBlock & { kind: 'tool_use' }> }): React.ReactElement {
  return (
...
export const AgentChatToolGroup = React.memo(function AgentChatToolGroup({
  blocks,
  defaultExpanded,
}: AgentChatToolGroupProps): React.ReactElement {
  const [expanded, setExpanded] = useState(
    defaultExpanded ?? blocks.some((b) => b.status === 'running'),
  );
  useEffect(() => { if (!defaultExpanded) setExpanded(false); }, [defaultExpanded]);
  ...
  return (
    <div className="my-1.5 rounded-md border border-border-semantic bg-surface-raised">
      ...
      <div className="agent-chat-tool-expand" data-collapsed={expanded ? 'false' : 'true'}>
        <ToolGroupCards blocks={blocks} />
      </div>
    </div>
  );
});

function ToolGroupStatusIcon({ allComplete }: { allComplete: boolean }): React.ReactElement {
  if (!allComplete) {
    return (
...
function ToolGroupHeader({
  expanded, allComplete, summaryText, count, onToggle,
}: {
  expanded: boolean; allComplete: boolean; summaryText: string; count: number; onToggle: () => void;
}): React.ReactElement {
  return (
    <button onClick={onToggle} className="flex w-full items-center gap-2 px-2.5 py-1.5 text-left text-xs transition-colors duration-100 hover:opacity-80">
      ...
      <ToolGroupStatusIcon allComplete={allComplete} />
