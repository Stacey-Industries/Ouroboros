  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 text-sm text-text-semantic-secondary space-y-3">
      ...
      <ol className="list-decimal list-inside space-y-1.5 text-text-semantic-primary">
        <li>
          Copy the content using the <strong>Copy</strong> button on the card.
        </li>
        <li>
          Save it as a <code className={mono}>.sh</code> file inside{' '}
          <code className={mono}>
            {'~/.claude/hooks/'}
            {eventType}/
          </code>
        ...
        <li>
          Make it executable: <code className={mono}>chmod +x &lt;file&gt;.sh</code>
        </li>
  ...
  return (
    <div className="flex flex-col h-full bg-surface-panel">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border-subtle shrink-0">
        <span className="text-text-semantic-primary font-medium text-sm">Awesome Ouroboros</span>
        <button
          onClick={onClose}
      ...
      {hookEntry && <HookInstructions entry={hookEntry} onClose={clearHookEntry} />}
    </div>
  );
}
...
export function AwesomeRefPanel({
  isOpen,
  onClose,
}: AwesomeRefPanelProps): React.ReactElement | null {
