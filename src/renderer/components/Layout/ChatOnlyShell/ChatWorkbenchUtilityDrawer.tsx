...
interface DrawerHeaderProps { activeTab: ChatWorkbenchUtilityTab; onClose: () => void }
  ...
  return (
    <header className="flex items-center gap-2 border-b border-stroke-default px-3 py-3">
      <div className="min-w-0 flex-1">
        <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-text-semantic-tertiary">Utility Drawer</div>
        <div className="mt-1 text-sm text-text-semantic-primary">{tabLabel(activeTab)}</div>
      </div>
      <button type="button" className="rounded border border-stroke-default bg-surface-panel px-2 py-1 text-xs text-text-semantic-secondary transition-colors hover:bg-surface-hover hover:text-text-semantic-primary"
        onClick={onClose} data-testid="chat-workbench-utility-close">Close</button>
    </header>
  );
}
...
export function ChatWorkbenchUtilityDrawer({ activeTab, onSelectTab, onClose }: ChatWorkbenchUtilityDrawerProps): React.ReactElement {
  const counts = useTabCounts();
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-stroke-default bg-surface-panel/95"
