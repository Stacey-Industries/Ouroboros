...
import {
  closeButtonStyle,
  excerptCountStyle,
  multiBufferIconStyle,
  multiBufferLabelStyle,
  multiBufferTabActiveStyle,
  multiBufferTabStyle,
  newMultiBufferButtonStyle,
  renameInputStyle,
...
export function useEditorTabActions(
  ...
  const handleCloseMultiBuffer = useCallback(
    (id: string) => {
      closeMultiBuffer(id);
...
function MultiBufferTabContent({
  buffer,
  isRenaming,
  renameValue,
  setRenameValue,
...
function useTabInteractionHandlers(buffer: MultiBufferTab, onActivate: (id: string) => void, onClose: (id: string) => void) {
  ...
  return { handleAuxClick, handleKeyDown, handleCloseClick };
}

export function MultiBufferTabItem({ buffer, isActive, onActivate, onClose, onRename }: { buffer: MultiBufferTab; isActive: boolean; onActivate: (id: string) => void; onClose: (id: string) => void; onRename: (id: string, name: string) => void }): React.ReactElement {
  const rename = useMultiBufferTabRename(buffer, onRename);
  const { handleAuxClick, handleKeyDown, handleCloseClick } = useTabInteractionHandlers(buffer, onActivate, onClose);
  return (
    ...
    >
      ...
      <MultiBufferTabContent buffer={buffer} isRenaming={rename.isRenaming} renameValue={rename.renameValue}
        setRenameValue={rename.setRenameValue} inputRef={rename.inputRef}
        commitRename={rename.commitRename} handleRenameKeyDown={rename.handleRenameKeyDown}
      />
...
export function MultiBufferTabs({
  buffers,
  activeId,
  onActivate,
  onClose,
