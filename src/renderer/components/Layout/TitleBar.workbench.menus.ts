...
function buildWorkbenchFileMenu(opts: WorkbenchMenuOptions): MenuDefinition {
  ...
  return {
    label: 'File',
    items: [
      { label: 'New Session', shortcut: 'Ctrl+Shift+N', action: () => dispatchEv(WORKBENCH_NEW_SESSION_EVENT) },
      { label: 'New Chat in Active Session', shortcut: 'Ctrl+N', action: () => dispatchEv(WORKBENCH_NEW_CHAT_EVENT) },
      SEPARATOR,
      { label: 'Open Project', shortcut: 'Ctrl+O', action: () => dispatchEv(WORKBENCH_OPEN_PROJECT_EVENT) },
      { label: 'Switch Project', submenu: buildSwitchProjectSubmenu(recents) },
      SEPARATOR,
      { label: 'Exit Chat Mode', shortcut: 'Ctrl+Alt+I', action: () => dispatchEv(TOGGLE_IMMERSIVE_CHAT_EVENT) },
    ],
  };
}
...
function buildWorkbenchEditMenu(): MenuDefinition {
  return {
    ...
    items: [
      ...
      SEPARATOR,
      { label: 'Find in Chat', shortcut: 'Ctrl+F', action: () => dispatchEv(WORKBENCH_OPEN_CHAT_SEARCH_EVENT) },
      { label: 'Find Next', shortcut: 'F3', action: () => dispatchEv(WORKBENCH_FIND_NEXT_EVENT) },
      { label: 'Find Previous', shortcut: 'Shift+F3', action: () => dispatchEv(WORKBENCH_FIND_PREV_EVENT) },
    ],
  };
}
...
function buildWorkbenchViewMenu(): MenuDefinition {
  return {
    label: 'View',
    items: [
      { label: 'Toggle Outer Rail', shortcut: 'Ctrl+B', action: () => dispatchEv(WORKBENCH_TOGGLE_OUTER_RAIL_EVENT) },
      { label: 'Toggle Inner Sidebar', shortcut: 'Ctrl+\\', action: () => dispatchEv(WORKBENCH_TOGGLE_INNER_SIDEBAR_EVENT) },
      SEPARATOR,
      { label: 'Toggle Utility Drawer', action: () => dispatchEv(WORKBENCH_TOGGLE_UTILITY_DRAWER_EVENT) },
      { label: 'Toggle Terminal Dock', shortcut: 'Ctrl+J', action: () => dispatchEv(WORKBENCH_TOGGLE_TERMINAL_DOCK_EVENT) },
      { label: 'Toggle Artifact Pane', action: () => dispatchEv(WORKBENCH_TOGGLE_ARTIFACT_PANE_EVENT) },
      SEPARATOR,
      { label: 'Switch to IDE Shell', shortcut: 'Ctrl+Alt+I', action: () => dispatchEv(TOGGLE_IMMERSIVE_CHAT_EVENT) },
    ],
  };
}
...
function buildWorkbenchToolsMenu(): MenuDefinition {
  return {
    label: 'Tools',
    items: [
      { label: 'Settings', shortcut: 'Ctrl+,', action: () => dispatchEv(OPEN_SETTINGS_PANEL_EVENT) },
...
function buildWorkbenchHelpMenu(): MenuDefinition {
  return {
    ...
    items: [
      ...
      {
        label: 'Report Issue',
        action: () => window.electronAPI?.app?.openExternal?.('https://github.com/hesnotsoharry/Ouroboros/issues'),
      },
    ],
  };
}
