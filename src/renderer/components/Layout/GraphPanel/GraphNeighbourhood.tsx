...
function NodeRow({ node, relation }: { node: RawGraphNode; relation: string }): React.ReactElement {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1 text-xs">
      <span className="shrink-0 text-[10px] text-text-semantic-muted w-12 text-right">
        {relation}
      </span>
...
function NodeSection({
  ...
  return (
    <>
      ...
      {nodes.map((node) => (
        <NodeRow key={node.id} node={node} relation={relation} />
      ))}
    </>
  );
}
    ...
    <div className="max-h-80 overflow-y-auto py-1">
      ...
      {empty && (
        <div className="px-3 py-4 text-xs text-text-semantic-muted">
          No neighbours found at this depth.
        </div>
      )}
    </div>
  );
...
function NeighbourhoodContent({
  ...
  if (!data.success)
    return (
      <div className="px-3 py-4 text-xs text-status-error">{data.error ?? 'Unknown error'}</div>
    );
...
export function GraphNeighbourhood({
  ...
  onClose,
  enabled,
}: GraphNeighbourhoodProps): React.ReactElement | null {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2">
        ...
        <button
          className="text-text-semantic-faint hover:text-text-semantic-primary transition-colors"
          onClick={onClose}
          aria-label="Close neighbourhood overlay"
        >
          ×
        </button>
      </div>
