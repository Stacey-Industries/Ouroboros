  ...
  return (
    <div className="border-t border-border-semantic" style={rowStyle}>
      <div style={{ display: 'flex', gap: '6px' }}>
        ...
        <ActionButton onClick={() => setShowPromote((p) => !p)} title="Save current layout as a named preset">Save as…</ActionButton>
      </div>
      {showPromote && <PromoteInput name={name} onChange={setName} onConfirm={handlePromote} onCancel={() => { setShowPromote(false); setName(''); }} />}
    </div>
  );
}
...
function PromoteInput({ name, onChange, onConfirm, onCancel }: { name: string; onChange: (v: string) => void; onConfirm: () => void; onCancel: () => void }): React.ReactElement {
  const hasTrim = Boolean(name.trim());
  return (
    <div style={{ display: 'flex', gap: '4px' }}>
      ...
      />
      <button type="button" onClick={onConfirm} disabled={!hasTrim}
        className="bg-interactive-accent text-text-semantic-on-accent"
        style={{ padding: '2px 8px', fontSize: '11px', borderRadius: '4px', cursor: hasTrim ? 'pointer' : 'default', opacity: hasTrim ? 1 : 0.5 }}
      >
        Save
      </button>
    </div>
