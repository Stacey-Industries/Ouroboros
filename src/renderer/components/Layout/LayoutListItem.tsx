...
export function LayoutListItem({ layout, isActive, onSelect, onUpdate, onDelete }: LayoutListItemProps): React.ReactElement {
  return (
    ...
    >
      ...
      {!layout.builtIn && <LayoutActions name={layout.name} onUpdate={onUpdate} onDelete={onDelete} />}
    </div>
  );
}

function LayoutRadioButton({ layout, isActive, onSelect }: { layout: WorkspaceLayout; isActive: boolean; onSelect: (l: WorkspaceLayout) => void }): React.ReactElement {
  return (
    <button
...
function LayoutActions({ name, onUpdate, onDelete }: { name: string; onUpdate: (n: string) => void; onDelete: (n: string) => void }): React.ReactElement {
  return (
    <div style={{ display: 'flex', gap: '2px', flexShrink: 0 }}>
      <ActionButton title="Update with current layout" onClick={() => onUpdate(name)} hoverColor="var(--text)" hoverBg="rgba(128,128,128,0.15)">
        <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M8 1v6m0 0l2.5-2.5M8 7L5.5 4.5" /><path d="M2 10v3a1 1 0 001 1h10a1 1 0 001-1v-3" />
        </svg>
      </ActionButton>
      <ActionButton title="Delete layout" onClick={() => onDelete(name)} hoverColor="var(--error, #f85149)" hoverBg="rgba(248,81,73,0.1)">
        <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function ActionButton({ title, onClick, hoverColor, hoverBg, children }: { title: string; onClick: () => void; hoverColor: string; hoverBg: string; children: React.ReactNode }): React.ReactElement {
  return (
    <button
      title={title}
