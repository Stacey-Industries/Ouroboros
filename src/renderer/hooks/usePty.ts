...
export function usePty(): UsePtyReturn {
  ...
  const write = useCallback((sid: string, d: string) => {
    void window.electronAPI.pty.write(sid, d);
  }, []);
  const resize = useCallback((sid: string, c: number, r: number) => {
    void window.electronAPI.pty.resize(sid, c, r);
  }, []);
  const kill = useCallback((sid: string) => {
    void window.electronAPI.pty.kill(sid);
  }, []);
  const onData = useCallback(
    (sessionId: string, callback: (data: string) => void) =>
      makeTrackedSubscriber<string>(cleanupsRef, window.electronAPI.pty.onData)(
        sessionId,
        callback,
      ),
    [],
  ...
  const onExit = useCallback(
    (sessionId: string, callback: (result: ExitResult) => void) =>
      makeTrackedSubscriber<ExitResult>(cleanupsRef, window.electronAPI.pty.onExit)(
        sessionId,
        callback,
      ),
    [],
