...
function renderDrain(initial: ConnectionState) {
  return renderHook(({ state }: { state: ConnectionState }) => useDispatchReconnectDrain(state), {
...
describe('useDispatchReconnectDrain', () => {
  ...
  it('shows toast when sent > 0', async () => {
    ...
    expect(mockToast).toHaveBeenCalledWith(expect.stringContaining('3 dispatches sent'), 'info');
  });
  ...
  it('shows toast when failed > 0', async () => {
    ...
    expect(mockToast).toHaveBeenCalledWith(expect.stringContaining('2 failed'), 'info');
  });
  ...
  it('shows toast when lost > 0', async () => {
    ...
    expect(mockToast).toHaveBeenCalledWith(expect.stringContaining('1 already processed'), 'info');
  });
