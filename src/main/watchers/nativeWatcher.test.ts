...
async function makeTmpDir(): Promise<string> {
  const dir = path.join(os.tmpdir(), `nwtest-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('watchRecursive', () => {
  ...
  it('emits a create event when a file is added at the root', async () => {
    ...
    try {
      ...
      expect(events.some((e) => e.path === target && e.type === 'create')).toBe(true);
    } finally {
      await sub.close();
    }
  });
  ...
  it('respects ignore globs — matching paths produce no events', async () => {
    ...
    const events: WatchEvent[] = [];
    const sub = await watchRecursive(testDir, { ignore: ['**/skipme/**'] }, (e) => events.push(e));
    try {
