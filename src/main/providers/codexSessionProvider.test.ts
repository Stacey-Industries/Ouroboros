...
const baseCodexSettings: CodexCliSettings = {
  model: '',
...
beforeEach(() => {
  vi.clearAllMocks();
  vi.mocked(getConfigValue).mockReturnValue(baseCodexSettings as ReturnType<typeof getConfigValue>);
});
...
describe('spawn', () => {
  ...
  it('passes model from config as a cliArg when set', async () => {
    ...
    expect(spawnCodexExecProcess).toHaveBeenCalledWith(
      expect.objectContaining({
        cliArgs: expect.arrayContaining(['--model', 'gpt-5.5']),
      }),
    );
  });

  it('passes the bypass flag to codex exec when bypass is enabled', async () => {
    vi.mocked(getConfigValue).mockReturnValue({
      ...baseCodexSettings,
    ...
    expect(spawnCodexExecProcess).toHaveBeenCalledWith(
      expect.objectContaining({
        cliArgs: expect.arrayContaining(['--dangerously-bypass-approvals-and-sandbox']),
      }),
    );
  });
});
