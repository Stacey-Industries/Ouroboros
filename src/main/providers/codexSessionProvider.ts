...
function makeEvent(type: SessionEvent['type'], sessionId: string, payload: unknown): SessionEvent {
...
function translateCodexEvent(event: CodexExecEvent, sessionId: string): SessionEvent | null {
  if (event.type === 'turn.completed')
    return translateTurnCompleted(event as CodexTurnCompletedEvent, sessionId);
  ...
  if (event.type === 'item.completed')
    return translateItemCompleted(event as CodexItemCompletedEvent, sessionId);
  if (
...
function handleCodexResult(
  ...
  publishEvent(sessionId, makeEvent('completion', sessionId, { threadId: result.threadId }));
}

async function spawnCodexSession(opts: SpawnOptions): Promise<SessionHandle> {
  const settings = getConfigValue('codexCliSettings');
  const cliArgs = buildCodexCliArgs(settings, 'exec');
  ...
  handle.result
    .then((result) => handleCodexResult(opts.sessionId, result))
    .catch((err: Error) => {
  ...
  return { id: opts.sessionId, providerId: 'codex', ptySessionId: opts.sessionId, startedAt: Date.now(), status: 'starting' };
}
...
export class CodexSessionProvider implements SessionProvider {
  ...
  async send(handle: SessionHandle, text: string): Promise<void> {
    ...
    log.warn(
      `[CodexSessionProvider] send() no-op for exec session ${handle.id} (text length: ${text.length})`,
    );
  }
