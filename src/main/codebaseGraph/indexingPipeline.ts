...
async function discoverFiles(
  ...
  const ctx: WalkContext = { projectRoot, ig, maxSize: options.maxFileSize ?? 512 * 1024, maxFiles: options.maxFiles ?? 10000, files }
...
async function filterChangedFiles(
  ...
  for (const file of files) {
    ...
    if (existing && existing.content_hash === hash) {
      db.upsertFileHash({
        project: projectName, rel_path: file.relativePath, content_hash: hash,
        mtime_ns: Math.floor(file.mtimeMs * 1e6), size: file.sizeBytes,
      })
      unchanged.push(file.relativePath)
      continue
    }
...
export class IndexingPipeline {
  ...
  private runAllPasses(
    ...
    this.db.transaction(() => {
      ...
      report('git_history')
      gitCoChangePass(this.db, projectName, projectRoot)
    })
  }

  private finalizeIndex(projectName: string, options: IndexingOptions, indexedFiles: IndexedFile[]): { nodesCreated: number; edgesCreated: number } {
    for (const file of indexedFiles) {
      this.db.upsertFileHash({
        project: projectName, rel_path: file.relativePath, content_hash: file.contentHash,
        mtime_ns: Math.floor(file.mtimeMs * 1e6), size: file.sizeBytes,
      })
    }
    const nodesCreated = this.db.getNodeCount(projectName)
    const edgesCreated = this.db.getEdgeCount(projectName)
    this.db.upsertProject({ name: projectName, root_path: options.projectRoot, indexed_at: Date.now(), node_count: nodesCreated, edge_count: edgesCreated })
  ...
  private async runIndex(
    ...
    const { filesToProcess, isIncrementalRun } = await this.resolveFilesToProcess(isIncremental, projectName, allFiles)
    this.db.upsertProject({ name: projectName, root_path: options.projectRoot, indexed_at: Date.now(), node_count: 0, edge_count: 0 })

    report('parsing')
    const indexedFiles = await parsePass(this.parser, filesToProcess, (processed) => {
      progress.filesProcessed = processed; report('parsing')
    })

    const structureFiles = isIncrementalRun ? filesToProcess : allFiles
    this.runAllPasses({ projectName, projectRoot: options.projectRoot }, indexedFiles, structureFiles, report)
    report('finalizing')
    const { nodesCreated, edgesCreated } = this.finalizeIndex(projectName, options, indexedFiles)

    return { projectName, success: true, filesIndexed: indexedFiles.length, filesSkipped: allFiles.length - filesToProcess.length, nodesCreated, edgesCreated, errors: progress.errors, durationMs: Date.now() - startTime, incremental: isIncrementalRun }
  }
  ...
  async index(options: IndexingOptions): Promise<IndexingResult> {
    ...
    const progress: IndexingProgress = {
      phase: 'discovery', filesTotal: 0, filesProcessed: 0,
      nodesCreated: 0, edgesCreated: 0, errors, startedAt: startTime, elapsedMs: 0,
    }
    const report = (phase: string): void => {
      progress.phase = phase; progress.elapsedMs = Date.now() - startTime; progress.errors = errors
    ...
    try {
      ...
      return { projectName, success: false, filesIndexed: 0, filesSkipped: 0, nodesCreated: 0, edgesCreated: 0, errors, durationMs: Date.now() - startTime, incremental: false }
    }
  }
