...
export async function listCodexModels(): Promise<CodexModelOption[]> {
  try {
    ...
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- cachePath is derived from os.homedir() + fixed suffix
...
export function buildCodexCliArgs(
  ...
  pushSandboxArgs(args, settings, mode);
  if (settings.search) {
    args.push('--search');
  }
