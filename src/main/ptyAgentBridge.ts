 ...
 */
export function subscribeSessionEvents(sessionId: string, cb: SessionEventCallback): () => void {
...
function maybeCacheSystemPrompt(sessionId: string, event: StreamJsonEvent): void {
  ...
  const text =
    typeof raw['system_prompt'] === 'string' ? raw['system_prompt'] : JSON.stringify(event);
