...
function truncateSig(raw: string): string {
  ...
  return normalised.length > MAX_SIGNATURE_LENGTH
    ? normalised.slice(0, MAX_SIGNATURE_LENGTH)
    : normalised;
}
...
export function parseConstExport(
  ...
  if (isArrowFn || isFunctionKw) {
    return {
      name,
      kind: 'function',
      signature: extractSignature(lines, i, afterName),
      isDefault: false,
      line: i + 1,
    };
  }
...
export function parseExportSpecifiers(specifiers: string, lineNumber: number): ExtractedSymbol[] {
