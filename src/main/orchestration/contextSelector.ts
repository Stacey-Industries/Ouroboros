...
async function applyKeywordReasons(
  candidates: Map<string, MutableCandidate>,
...
export async function selectContextFiles(options: {
  ...
  const liveIdeState =
    ...
    (await collectLiveIdeState(workspaceRoots, selection.selectedFiles, snapshots));
  ...
  if (cfg?.learnedRanker === true) {
    return buildResult({
      ...baseOpts,
      rankedFilesOverride: classifierRankCandidates(candidates, request),
    });
  }
