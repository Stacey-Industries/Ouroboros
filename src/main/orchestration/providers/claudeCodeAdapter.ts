...
import type { StreamJsonEvent, StreamJsonProcessHandle, StreamJsonResultEvent } from './streamJsonTypes'
...
async function materializeAttachments(attachments: ImageAttachment[]): Promise<{ goalSuffix: string; tempPaths: string[] }> {
...
function buildStopReasonMessage(result: StreamJsonResultEvent): string | null {
  ...
  if (result.stop_reason === 'max_tokens') return '**Agent stopped** — hit output token limit (stop_reason: max_tokens)'
  if (result.stop_reason && result.stop_reason !== 'end_turn') return `**Agent stopped** — unexpected stop_reason: \`${result.stop_reason}\``
  return null
}
...
function launchHeadless(args: {
  ...
  const handle = spawnStreamJsonProcess({ prompt: args.prompt, cwd: args.cwd, model: args.settings.model || undefined, permissionMode: args.settings.permissionMode !== 'default' ? args.settings.permissionMode : undefined, dangerouslySkipPermissions: args.settings.dangerouslySkipPermissions || undefined, resumeSessionId: args.resumeSessionId || undefined, continueSession: args.continueSession || undefined, effort: args.effort || undefined, env: args.providerEnv, onEvent: handler })
...
function resolveEffectiveSettings(context: ProviderLaunchContext | ProviderResumeContext, settings: ClaudeCliSettings): {
  resolvedModel: string | undefined
  ...
  return { resolvedModel, effort, effectiveSettings: { ...settings, model: isProviderRouted ? '' : (resolvedModel ?? ''), permissionMode }, providerEnv, isProviderRouted }
}

function buildPlaceholderHandle(taskId: string): { placeholder: StreamJsonProcessHandle; getCancelledBeforeLaunch: () => boolean } {
  let cancelledBeforeLaunch = false
  const placeholder: StreamJsonProcessHandle = { result: null as unknown as Promise<StreamJsonResultEvent>, kill: () => { const realHandle = activeProcesses.get(taskId); if (realHandle && realHandle !== placeholder) { realHandle.kill(); return } cancelledBeforeLaunch = true }, pid: undefined, sessionId: null }
  return { placeholder, getCancelledBeforeLaunch: () => cancelledBeforeLaunch }
}

function resolveTokenUsage(result: StreamJsonResultEvent | null, tracked: { inputTokens: number; outputTokens: number }): { inputTokens: number; outputTokens: number } | undefined {
  ...
  return usage ? { inputTokens: (usage.input_tokens ?? 0) + (usage.cache_creation_input_tokens ?? 0), outputTokens: usage.output_tokens ?? 0 } : undefined
}
...
function handleLaunchSuccess(result: StreamJsonResultEvent | null, args: CompletionArgs): void {
  ...
  if (diagnostic) {
    ...
    args.sink.emit({ provider: 'claude-code', status: 'streaming', message: diagnostic, timestamp: Date.now(), session: args.sessionRef, contentBlock: { blockIndex: args.getNextGlobalBlockIndex(), blockType: 'text', textDelta: diagnostic } })
  }
  args.sink.emit({
    provider: 'claude-code',
    status: 'completed',
...
function handleLaunchError(error: unknown, args: CompletionArgs): void {
  ...
  if (wasCancelled) {
    ...
    args.sink.emit({ provider: 'claude-code', status: 'cancelled', message: 'Task cancelled by user', timestamp: Date.now(), session: args.sessionRef })
    return
  }
  ...
  args.sink.emit({ provider: 'claude-code', status: 'streaming', message: errorDiagnostic, timestamp: Date.now(), session: args.sessionRef, contentBlock: { blockIndex: args.getNextGlobalBlockIndex(), blockType: 'text', textDelta: errorDiagnostic } })
  args.sink.emit({ provider: 'claude-code', status: 'failed', message: errorMsg, timestamp: Date.now(), session: args.sessionRef })
}
  ...
  return (async () => {
    ...
    if (args.context.request.goalAttachments?.length) {
      try {
        ...
        console.error('[claude-code] failed to materialize attachments — images will be omitted:', err)
      }
    }
    if (args.getCancelledBeforeLaunch()) { activeProcesses.delete(args.context.taskId); return null }
    return launchHeadless({ context: args.context, prompt: buildInitialPrompt(args.context, goalSuffix, Boolean(args.effectiveResumeSessionId), args.resolvedModel ?? ''), cwd: args.cwd, settings: args.effectiveSettings, sessionRef: args.sessionRef, sink: args.sink, resumeSessionId: args.effectiveResumeSessionId, continueSession: 'providerSession' in args.context && !args.effectiveResumeSessionId ? true : undefined, effort: args.effort, providerEnv: args.isProviderRouted ? args.providerEnv : undefined, eventHandler: args.eventHandler }).result
  })()
}

function launchClaude(context: ProviderLaunchContext | ProviderResumeContext, sink: ProviderProgressSink, resumeSessionId?: string): ProviderLaunchResult {
  ...
  const { resolvedModel, effort, effectiveSettings, providerEnv, isProviderRouted } = resolveEffectiveSettings(context, settings)
  ...
  const sessionRef = createProviderSessionReference('claude-code', { requestId, sessionId: 'providerSession' in context ? context.providerSession?.sessionId : undefined, externalTaskId: context.taskId })
  ...
  const completionArgs: CompletionArgs = { taskId: context.taskId, sessionRef, sink, invocationTempPaths, resolvedModel, getNextGlobalBlockIndex, getCumulativeUsage }
  scheduleClaudeLaunch({ context, cwd, sessionRef, sink, resolvedModel, effectiveResumeSessionId, effectiveSettings, eventHandler, effort, providerEnv, isProviderRouted, getCancelledBeforeLaunch, invocationTempPaths }).then(
    (result) => handleLaunchSuccess(result, completionArgs),
    (error) => handleLaunchError(error, completionArgs),
  )
  const submittedAt = Date.now()
  sink.emit({ provider: 'claude-code', status: 'queued', message: 'Claude Code session started', timestamp: submittedAt, session: sessionRef })
  return { session: sessionRef, artifact: createProviderArtifact({ provider: 'claude-code', status: 'streaming', session: sessionRef, submittedAt }) }
}

function cancelPtySession(targetId: string): boolean {
  const agentPty = activeAgentPtySessions.get(targetId)
  if (agentPty) { agentPty.bridge.dispose(); killPty(agentPty.ptySessionId); activeAgentPtySessions.delete(targetId); return true }
  for (const [key, entry] of activeAgentPtySessions) {
    if (entry.ptySessionId === targetId || key === targetId) {
      entry.bridge.dispose(); killPty(entry.ptySessionId); activeAgentPtySessions.delete(key); return true
    }
  }
  return false
}
...
function cancelHeadlessProcess(targetId: string): boolean {
  ...
  if (handle) { cancelledTasks.add(targetId); handle.kill(); activeProcesses.delete(targetId); return true }
  for (const [key, proc] of activeProcesses) {
    if (proc.sessionId === targetId || key === targetId) {
      cancelledTasks.add(key); proc.kill(); activeProcesses.delete(key); return true
    }
  }
  return false
}
