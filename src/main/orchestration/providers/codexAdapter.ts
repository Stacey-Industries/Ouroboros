...
import { applyCodexPermissionModeOverride, buildCodexCliArgs, mapEffortToCodexReasoning } from '../../codex'
...
async function materializeAttachments(attachments: ImageAttachment[]): Promise<{ imagePaths: string[] }> {
...
function resolveSettings(context: ProviderLaunchContext | ProviderResumeContext): { cliArgs: string[]; model: string } {
...
function handleLaunchSuccess(
  ...
  if (!result) {
    args.sink.emit({ provider: 'codex', status: 'cancelled', message: 'Task cancelled by user', timestamp: Date.now(), session: args.sessionRef })
    return
  }
...
function handleLaunchError(error: unknown, args: CodexCompletionArgs): void {
  ...
  args.sink.emit({
    ...
    timestamp: Date.now(),
    session: args.sessionRef,
    contentBlock: { blockIndex: args.getNextBlockIndex(), blockType: 'text', textDelta: `\n\n---\n**Codex stopped** - ${errorMessage}` },
  })
  args.sink.emit({ provider: 'codex', status: 'failed', message: errorMessage, timestamp: Date.now(), session: args.sessionRef })
}
...
function launchCodex(context: ProviderLaunchContext | ProviderResumeContext, sink: ProviderProgressSink, resumeThreadId?: string): ProviderLaunchResult {
  ...
  const sessionRef = createProviderSessionReference('codex', { requestId, sessionId: resumeThreadId || ('providerSession' in context ? context.providerSession?.sessionId : undefined), externalTaskId: context.taskId })
  sink.emit({ provider: 'codex', status: 'queued', message: 'Launching Codex session', timestamp: Date.now(), session: sessionRef })
  const { handler: eventHandler, getNextBlockIndex, getUsage } = buildCodexEventHandler(sink, sessionRef)
  ...
  const placeholder: CodexExecProcessHandle = {
    ...
    kill: () => { const realHandle = activeProcesses.get(context.taskId); if (realHandle && realHandle !== placeholder) { realHandle.kill(); return } cancelledBeforeLaunch = true },
    threadId: sessionRef.sessionId ?? null,
  }
  activeProcesses.set(context.taskId, placeholder)
  const resultPromise = scheduleCodexLaunch({ context, cwd, cliArgs, model, resumeThreadId, invocationTempPaths, getCancelledBeforeLaunch: () => cancelledBeforeLaunch, eventHandler })
  const completionArgs: CodexCompletionArgs = { taskId: context.taskId, sessionRef, sink, getUsage, getNextBlockIndex, invocationTempPaths }
  resultPromise.then((result) => handleLaunchSuccess(result, completionArgs), (error) => handleLaunchError(error, completionArgs))
  sink.emit({ provider: 'codex', status: 'queued', message: 'Codex session started', timestamp: Date.now(), session: sessionRef })
  return { session: sessionRef, artifact: createProviderArtifact({ provider: 'codex', status: 'streaming', session: sessionRef, submittedAt: Date.now() }) }
}
