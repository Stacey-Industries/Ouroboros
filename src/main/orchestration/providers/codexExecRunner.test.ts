...
function registerSpawnFailureTests(): void {
  ...
  it('rejects on non-zero exit code with stderr output', async () => {
    ...
    await expect(handle.result).rejects.toThrow(
      'Codex exec exited with code 1: Something went wrong',
    );
  });
}

function registerSpawnUtilityTests(): void {
...
describe('codexExecRunner', () => {
  ...
  describe('buildCodexExecArgs', registerArgTests);
  describe('spawnCodexExecProcess', () => {
    registerSpawnParsingTests();
    registerSpawnFailureTests();
    registerSpawnUtilityTests();
  });
});
