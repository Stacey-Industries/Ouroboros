...
function emitSubToolEvent(
  parent: { name: string; globalIndex: number },
  sub: {
    name: string;
    ...
    contentBlock: {
      ...
      toolActivity: {
        ...
        subToolActivity: {
          ...
          inputSummary: sub.inputSummary,
          editSummary: sub.editSummary,
          output: sub.output,
          subToolId: sub.subToolId,
        },
      },
    },
  });
...
function processToolUse(
  ...
  parentCtx: { parentId: string; name: string; globalIndex: number },
  ctx: EmitCtx,
): void {
...
function processToolResult(
  state: SubagentState,
  block: StreamJsonToolResultBlock,
  parent: { name: string; globalIndex: number },
  ctx: EmitCtx,
): void {
  ...
  emitSubToolEvent(
    parent,
    { name: '', status: 'complete', subToolId: mapping.subToolId, output: raw || undefined },
    ctx,
  );
}
...
export function handleSubagentEvent(
  ...
  for (const block of event.message.content) {
    if (block.type === 'tool_use')
      processToolUse(state, block as StreamJsonToolUseBlock, parentCtx, ctx);
    else if (block.type === 'tool_result')
      processToolResult(state, block as StreamJsonToolResultBlock, parent, ctx);
  }
}
