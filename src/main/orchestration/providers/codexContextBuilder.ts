...
export function buildDiagnosticsSection(packet: ContextPacket): string {
  ...
  const lines: string[] = [
    `<diagnostics errors="${diagnostics.totalErrors}" warnings="${diagnostics.totalWarnings}">`,
  ];
  for (const file of diagnostics.files) {
    if (!file.messages || file.messages.length === 0) continue;
    for (const message of file.messages)
      lines.push(
        `${path.basename(file.filePath)}:${message.line} - ${message.severity}: ${message.message}`,
      );
  }
