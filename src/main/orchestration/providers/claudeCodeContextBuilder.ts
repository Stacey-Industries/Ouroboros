...
export function buildWorkspaceStateSection(packet: ContextPacket): string {
  ...
  if (dirtyCount > 0)
    lines.push(
      `Unsaved buffers (${dirtyCount}): ${packet.liveIdeState.dirtyFiles.map((f) => path.basename(f)).join(', ')}`,
    );
  lines.push('</workspace_state>');
  return lines.join('\n');
}

export function buildRelevantCodeSection(
  ...
  for (const file of packet.files.slice(0, fileLimit)) {
    const reasons = file.reasons
      ...
      .join('; ');
    lines.push(
      `<file path="${file.filePath}" score="${file.score}" confidence="${file.confidence}" reasons="${reasons}">`,
    );
...
export function buildDiagnosticsSection(packet: ContextPacket): string {
  ...
  const lines: string[] = [
    `<diagnostics errors="${diag.totalErrors}" warnings="${diag.totalWarnings}">`,
  ];
...
function appendExtraPacketSections(
  output: string,
  packet: ContextPacket,
  includeGraphSummary: boolean,
): string {
  let result = output;
  if (includeGraphSummary && packet.graphSummary) result += '\n\n' + packet.graphSummary;
  if (packet.sessionMemories) result += '\n\n' + packet.sessionMemories;
...
export function buildXmlContextBlock(
  ...
  return appendExtraPacketSections(output, packet, true);
}

// On resume turns the agent already has full conversation continuity via
// --resume; re-sending <relevant_code>, <project_structure>, <diagnostics>,
// <terminal_output>, and the graph summary every turn is pure duplication
// (agent has either read or can re-read these). We keep only <pinned_context>
// (user-controlled), <workspace_state> (cheap delta on git/dirty state),
// and the extras (session memories, skill/system instructions).
export function buildResumeContextBlock(context: ProviderResumeContext): string {
  const packet = context.contextPacket;
  if (!packet) return '';
  const sections: string[] = [];
  if (packet.pinnedContext)
...
export function buildInitialPrompt(
  ...
  lines.push(
    isResume
      ? buildResumeContextBlock(context as ProviderResumeContext)
      : buildXmlContextBlock(context, model),
  );
  return lines.join('\n');
}
...
function resolveToolInputSummary(inp: Record<string, unknown>): string | undefined {
  if (typeof inp.command === 'string')
    return inp.command.length > 200 ? inp.command.slice(0, 197) + '...' : inp.command;
  if (typeof inp.pattern === 'string')
    return `/${inp.pattern}/` + (typeof inp.glob === 'string' ? ` in ${inp.glob}` : '');
  if (typeof inp.description === 'string')
    return inp.description.length > 150 ? inp.description.slice(0, 147) + '...' : inp.description;
  return undefined;
}
...
export function extractToolDisplayFields(block: StreamJsonToolUseBlock): {
  ...
  if (typeof inp.old_string === 'string' && typeof inp.new_string === 'string') {
    result.editSummary = {
      oldLines: inp.old_string.split('\n').length,
      newLines: inp.new_string.split('\n').length,
    };
  }
  return result;
}
