...
function hasNewToolUseId(state: EventHandlerState, blocks: StreamJsonContentBlock[]): boolean {
  ...
  for (const block of blocks) {
    if (
      block.type === 'tool_use' &&
      !state.toolIdToGlobal.has((block as StreamJsonToolUseBlock).id)
    ) {
      return true;
    }
  }
...
function emitToolCompletion(
  info: { name: string; globalIndex: number },
  ...
  ctx.sink.emit({
    ...
    contentBlock: {
      ...
      blockType: 'tool_use',
      toolActivity: { name: info.name, status: 'complete', output },
    },
  });
}
...
function makeUsage(state: EventHandlerState) {
  return { inputTokens: state.lastTurnInputTokens, outputTokens: state.cumulativeOutputTokens };
}
...
function emitTextDelta(state: EventHandlerState, globalIdx: number, text: string, ctx: EmitCtx): void {
  ...
  ctx.sink.emit({
    provider: 'claude-code', status: 'streaming', message: delta,
    timestamp: ctx.now, session: ctx.sessionRef,
    contentBlock: { blockIndex: globalIdx, blockType: 'text', textDelta: delta },
    tokenUsage: makeUsage(state),
  });
...
function emitThinkingDelta(state: EventHandlerState, globalIdx: number, thinking: string, ctx: EmitCtx): void {
  ...
  if (delta.length === 0) return;
  ctx.sink.emit({
    provider: 'claude-code', status: 'streaming', message: '',
    timestamp: ctx.now, session: ctx.sessionRef,
...
function applyAssistantUsage(
  state: EventHandlerState,
  event: StreamJsonEvent & { type: 'assistant' },
): void {
...
function handleAssistantEvent(
  ...
  for (let i = state.localToGlobal.length; i < blocks.length; i++)
    state.localToGlobal.push(state.nextGlobalBlockIndex++);
...
function createInitialState(): EventHandlerState {
  return {
    nextGlobalBlockIndex: 0, localToGlobal: [], prevBlockTypes: [],
    emittedContentLengths: new Map(), toolIdToGlobal: new Map(),
    toolResultContents: new Map(), parentToolUseToGlobal: new Map(),
    subToolCounters: new Map(), subToolIdByToolUseId: new Map(), pendingCompletions: new Map(),
