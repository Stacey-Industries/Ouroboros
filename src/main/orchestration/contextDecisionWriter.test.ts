...
const BASE_PATH = path.join(path.sep + 'userData', 'context-decisions.jsonl');

function makeDeps(fileSize = 0): { deps: DecisionWriterDeps; appended: string[]; rotated: string[][]; unlinked: string[] } {
  ...
  const deps: DecisionWriterDeps = {
    ...
    appendLine: vi.fn(async (_fp, line) => { appended.push(line); }),
...
describe('createDecisionWriter — closeDecisionWriter', () => {
  ...
  it('flushes pending queue on close', async () => {
    ...
    expect(appended).toHaveLength(1);
    expect(JSON.parse(appended[0].trim())).toMatchObject({ id: 'pre-close' });
  });
...
describe('createDecisionWriter — rotation', () => {
  beforeEach(() => { vi.useFakeTimers(); });
  afterEach(() => { vi.useRealTimers(); });
  ...
  it('rotates current file to .1 when size exceeds 10 MB', async () => {
    ...
    const hasPrimaryRotation = rotated.some(
      ([src, dst]) =>
        src === BASE_PATH &&
        dst === path.join(dir, 'context-decisions.1.jsonl'),
    );
  ...
  it('unlinks the .3 rotation (purge oldest) when rotating over 10 MB', async () => {
    ...
    const dir = path.dirname(BASE_PATH);
    expect(unlinked).toContain(path.join(dir, 'context-decisions.3.jsonl'));
  });
});
