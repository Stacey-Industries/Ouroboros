...
import type { DiagnosticMessage, DiagnosticsFileSummary, DiagnosticsSummary } from './types';
...
function collectServerDiagnostics(
  server: {
    diagnosticsCache: Map<
      string,
      Array<{ severity: string; range: { startLine: number; startChar: number }; message: string }>
    >;
  },
  rootPath: string,
...
function collectDiagnostics(rootPath: string): {
  byFile: Map<string, DiagnosticsFileSummary>;
  messagesByFile: Map<string, DiagnosticMessage[]>;
} {
  ...
  for (const server of servers.values()) {
    if (
      normalizePathForCompare(server.root) !== normalizePathForCompare(rootPath) ||
...
function capMessagesPerFile(
  ...
  for (const [filePath, messages] of messagesByFile) {
    messages.sort(
      (l, r) =>
        (SEVERITY_PRIORITY[l.severity] ?? 3) - (SEVERITY_PRIORITY[r.severity] ?? 3) ||
        l.line - r.line,
    );
...
function capMessagesGlobal(files: DiagnosticsFileSummary[]): void {
  ...
  const sorted = [...files].sort(
    (l, r) =>
      r.errors - l.errors || r.warnings - l.warnings || l.filePath.localeCompare(r.filePath),
  );
 ...
 */
export function buildLspDiagnosticsSummary(
  rootPath: string,
  generatedAt: number,
): DiagnosticsSummary {
