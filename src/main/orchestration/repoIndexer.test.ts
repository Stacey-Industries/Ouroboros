...
async function buildDeterministicFixture(): Promise<{
  root: string;
  entryFile: string;
  snapshot: Awaited<ReturnType<typeof buildRepoIndexSnapshot>>;
}> {
  ...
  servers.set(
    ...
    createServer(root, entryFile, [
      ...
      },
      {
        ...
        severity: 'warning',
        range: { startLine: 1, startChar: 0, endLine: 1, endChar: 6 },
      },
    ]),
  );
  const snapshot = await buildRepoIndexSnapshot([root], {
    now: 123,
...
function registerDeterministicRepoFactsTest(): void {
  it('builds deterministic repo facts with imports and diagnostics summaries', async () => {
    ...
    expect(indexedEntry?.diagnostics).toEqual({
      errors: 1,
      warnings: 1,
      infos: 0,
      hints: 0,
    ...
    expect(snapshot.repoFacts.recentEdits).toEqual({
      files: snapshot.repoFacts.recentEdits.files,
      generatedAt: 123,
    });
  });
}
...
function registerRootCacheReuseTest(): void {
  it('reuses cached root snapshots when the caller supplies matching state keys', async () => {
    ...
    const second = await buildRepoIndexSnapshot([root], {
      now: 200,
      rootStateKeys: { [root]: first.roots[0].stateKey },
    });

    expect(second.cache.roots).toEqual([
      { rootPath: root, key: first.roots[0].stateKey, hit: true },
    ]);
...
function registerWorkspaceCacheReuseTest(): void {
  it('reuses cached workspace snapshots when the caller supplies the matching workspace state key', async () => {
    ...
    const second = await buildRepoIndexSnapshot([root], {
      now: 500,
      workspaceStateKey: first.cache.key,
    });
