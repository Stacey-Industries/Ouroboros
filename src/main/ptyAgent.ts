...
function buildAgentPtyClaudeArgs(options: AgentPtyOptions): { shell: string; args: string[] } {
  ...
  if (options.effort) {
    ...
    if (mapped !== undefined) { cliArgs.push('--max-turns', String(mapped)) }
    else if (options.effort !== 'max') { cliArgs.push('--max-turns', options.effort) }
  }
...
function createAgentBridgeWithResult(id: string, options: AgentPtyOptions): AgentBridgeSetup {
  ...
  bridge.dispose = () => {
    if (!settled) { settled = true; resolveResult(null) }
    originalDispose()
  }
  return { bridge, result }
}

function wireBridgeToProc(id: string, proc: pty.IPty, bridge: ReturnType<typeof createAgentBridge>): void {
  let earlyOutput = ''
...
export function spawnAgentPty(id: string, win: BrowserWindow, options: AgentPtyOptions): AgentPtyResult {
  if (sessions.has(id)) return { success: false, error: `Session ${id} already exists` }

  const { cwd, cols, rows } = resolveSpawnOptions({ cwd: options.cwd, cols: options.cols, rows: options.rows })
  ...
  try {
    ...
    setTimeout(() => {
      if (sessions.has(id)) { proc.write(options.prompt); proc.write(eofChar) }
    }, 150)
