  ...
  it('starts the metrics interval on subscribe and stops it on unsubscribe', async () => {
    ...
    perfMetrics.initializePerfMetrics({
      getActiveWindows: () => [
        {
          isDestroyed: () => false,
          webContents: { id: 7, send } as unknown as WebContents,
    ...
    expect(
      perfMetrics.subscribeToPerfMetrics({ sender: { id: 7 } } as unknown as IpcMainInvokeEvent),
    ).toEqual({ success: true });
    expect(setIntervalSpy).toHaveBeenCalledTimes(1);

    expect(
      perfMetrics.unsubscribeFromPerfMetrics({
        sender: { id: 7 },
...
describe('emitIndexerCompleted', () => {
  ...
  it('broadcasts perf:indexer-completed to live windows', async () => {
    ...
    const perfMetrics = await import('./perfMetrics');
    perfMetrics.initializePerfMetrics({
      getActiveWindows: () => [{ isDestroyed: () => false, webContents: { id: 9, send } } as never],
    });
  ...
  it('skips destroyed windows without throwing', async () => {
    ...
    const perfMetrics = await import('./perfMetrics');
    perfMetrics.initializePerfMetrics({
      getActiveWindows: () => [{ isDestroyed: () => true, webContents: { id: 10, send } } as never],
    });

    expect(() => {
      perfMetrics.emitIndexerCompleted({
        durationMs: 1,
        filesIndexed: 1,
        nodesCreated: 1,
        edgesCreated: 1,
