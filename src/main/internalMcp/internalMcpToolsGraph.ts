...
export const getCodebaseContextTool: McpToolDefinition = {
  ...
  async handler(args, workspaceRoot) {
    const maxModules =
      typeof args.maxModules === 'number' ? Math.max(1, Math.min(args.maxModules, 50)) : 15;
...
export const traceImportsTool: McpToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      moduleId: {
        type: 'string',
        description: 'The module ID to trace (e.g. "orchestration", "file-viewer")',
      },
      direction: {
        type: 'string',
...
export const detectChangesTool: McpToolDefinition = {
  ...
  async handler(_args, workspaceRoot) {
    ...
    for (const entry of changedModules) {
      ...
      const changedFilesForModule = filterChangedFilesForModule(
