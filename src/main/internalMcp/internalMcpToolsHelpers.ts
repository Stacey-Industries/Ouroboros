...
export function appendSymbolsSection(entry: ModuleContextEntry, lines: string[]): void {
  ...
  if (s.extractedSymbols && s.extractedSymbols.length > 0) {
    ...
    if (s.extractedSymbols.length > 20)
      lines.push(`... and ${s.extractedSymbols.length - 20} more`);
    lines.push('');
  }
...
export function appendModuleGroup(
  label: string,
  modules: RepoMap['modules'],
  lines: string[],
): void {
...
function appendModuleGroupsByPath(repoMap: RepoMap, lines: string[]): void {
  const rp = (prefix: string) =>
    repoMap.modules.filter((e) => e.structural.module.rootPath.startsWith(prefix));
  appendModuleGroup('Main Process', rp('src/main'), lines);
  appendModuleGroup('Preload', rp('src/preload'), lines);
  appendModuleGroup('Renderer', rp('src/renderer'), lines);
  appendModuleGroup(
    'Other',
    repoMap.modules.filter(
      (e) =>
...
export function formatArchitectureBody(repoMap: RepoMap): string[] {
  ...
  lines.push(
    `Generated: ${repoMap.generatedAt ? new Date(repoMap.generatedAt).toISOString() : 'unknown'}`,
  );
  lines.push('');
  lines.push('## Overview');
  ...
  lines.push('');
  appendModuleGroupsByPath(repoMap, lines);
  return lines;
}
...
export function formatSymbolSearchResults(
  ...
  for (const sym of results) {
    const loc = sym.filePath
      ? `${sym.filePath}:${sym.line ?? 0}`
      : `${sym.moduleId}:${sym.line ?? 0}`;
...
export function formatImportGraphLines(
  moduleId: string,
  repoMap: RepoMap,
  direction: string,
): string[] {
...
export function filterChangedFilesForModule(
  changedFiles:
    | Array<{ filePath: string; additions?: number; deletions?: number; status?: string }>
    | undefined,
  moduleRoot: string,
): ChangedFileInfo[] {
  return (
