...
export function registerClaudeMdHandlers(_senderWindow: SenderWindow): string[] {
  ...
  ipcMain.handle('claudeMd:generate', async (_event, projectRoot: string, options?: { fullSweep?: boolean }) => {
    try {
  ...
  ipcMain.handle('claudeMd:generateForDir', async (_event, projectRoot: string, dirPath: string) => {
    try {
