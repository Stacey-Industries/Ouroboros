...
import {
  fileExists, readJsonSafe, readTextSafe,
  DIR_PURPOSES, CONFIG_FILES,
  detectFramework, detectTestFramework, detectPatterns, detectCommonPatterns,
} from './contextDetectors'

// ─── Phase: collect dirs and configs ──────────────────────────────────────────

async function collectDirsAndConfigs(projectRoot: string, topEntries: string[], ctx: ProjectContext): Promise<void> {
...
async function scanNodeProject(projectRoot: string, ctx: ProjectContext): Promise<boolean> {
  ...
  ctx.language = allDeps['typescript'] || ctx.keyConfigs.includes('tsconfig.json') ? 'TypeScript' : 'JavaScript'
...
async function addExistingEntries(root: string, entries: string[], target: string[]): Promise<void> {
...
function collectBuildCommands(pkgJson: Record<string, unknown>, ctx: ProjectContext): void {
  ...
  if (!scripts) return
  const interesting = ['dev', 'start', 'build', 'test', 'lint', 'format', 'typecheck', 'check', 'preview', 'deploy']
...
async function scanPythonProject(root: string, ctx: ProjectContext): Promise<boolean> {
  ...
  await addExistingEntries(root, ['app.py', 'main.py', 'manage.py', 'wsgi.py', 'asgi.py'], ctx.entryPoints)
  ...
  await addDetectedPatterns(root, [
    ['poetry.lock', 'Poetry'],
