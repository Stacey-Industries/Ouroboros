...
import type { ProjectContext, ContextGenerateOptions } from './contextTypes'
...
function generateStructure(ctx: ProjectContext): string[] {
  const filtered = ctx.keyDirs.filter((d) => !['node_modules', 'dist', 'build', 'out'].includes(d.path))
...
export function generateClaudeMdContent(ctx: ProjectContext, options: ContextGenerateOptions = {}): string {
