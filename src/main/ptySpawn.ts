...
export function spawnClaudePty(
  ...
  try {
    ...
    if (options.initialPrompt) scheduleStartupCommand(id, proc, options.initialPrompt, 300)
...
export function spawnCodexPty(
  ...
  try {
    const proc = pty.spawn(launch.shell, launch.args, {
      name: 'xterm-256color', cols, rows, cwd, env: buildBaseEnv(options.env),
    })
    registerSession({ id, proc, cwd, shell: launch.shell, win })
    if (options.initialPrompt) scheduleStartupCommand(id, proc, options.initialPrompt, 500)
    notifyTerminalCreated(id, cwd)
    return { success: true }
  } catch (error) {
    cleanupSession(id)
