...
function stubSnapshot(snapshot: PromptDiffSnapshot | null): void {
  mockGetConfig.mockReturnValue(snapshot ? { lastSeenSnapshot: snapshot } : null);
}
...
describe('checkPromptChanged', () => {
  ...
  it('returns changed:false and saves snapshot on first run (no stored snapshot)', async () => {
    ...
    const [key, val] = mockSetConfig.mock.calls[0] as [
      string,
      { lastSeenSnapshot: PromptDiffSnapshot },
    ];
  ...
  it('returns changed:false when version and hash both match', async () => {
    ...
    const stored = (
      mockSetConfig.mock.calls[0] as [string, { lastSeenSnapshot: PromptDiffSnapshot }]
    )[1].lastSeenSnapshot;
  ...
  it('stores updated snapshot even when returning changed:false (sub-threshold)', async () => {
    ...
    expect(mockSetConfig).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        lastSeenSnapshot: expect.objectContaining({ cliVersion: '1.0.0' }),
      }),
    );
  });
});
