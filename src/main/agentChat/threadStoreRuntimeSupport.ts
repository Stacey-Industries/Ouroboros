...
export class AgentChatThreadStoreRuntime {
  ...
  async writeThread(thread: AgentChatThreadRecord): Promise<AgentChatThreadRecord> {
    ...
    await this.ensureThreadsDir()
    await fs.writeFile(this.getThreadFilePath(normalizedThread.id), JSON.stringify(normalizedThread, null, 2), 'utf-8')
  ...
  async updateTitleFromResponse(threadId: string, assistantContent: string): Promise<AgentChatThreadRecord | null> {
    const thread = await this.readThread(threadId)
    ...
    const currentTitleMatchesUserMessage =
      ...
      thread.title === firstUserMessage.content.split(/\r?\n/).map((l) => l.trim()).find((l) => l.length > 0) ||
      (firstUserMessage.content.trim().length > 79 &&
        thread.title === `${firstUserMessage.content.split(/\r?\n/).map((l) => l.trim()).find((l) => l.length > 0)?.slice(0, 79).trimEnd()}\u2026`)
