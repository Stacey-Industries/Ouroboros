...
export function formatCompactionSummary(summary: CompactionSummary): string {
  ...
  lines.push(`[End Summary — ${summary.turnsSummarized} turns compressed from ~${summary.originalTokenCount} tokens]`)
  return lines.join('\n')
}

const FILE_PATH_RE = /(?:^|\s)((?:\/|[a-zA-Z]:\\|src\/|\.\.?\/)\S+\.\w{1,10})/g
const ERROR_RE = /^.*\b(?:error|Error|ERROR|failed|Failed|FAILED|bug|exception|crash|TypeError|ReferenceError)\b.*$/gm
const TOOL_RE = /\b(Read|Edit|Write|Grep|Glob|Bash|MultiEdit|NotebookEdit|execute_command|read_file|edit_file|write_file|search_files|find_files|create_file)\b/g
...
export function buildInlineSummary(
  ...
  const errorLines = rawErrors.length <= 5 ? rawErrors : [...rawErrors.slice(0, 5), `...and ${rawErrors.length - 5} more`]
...
export function buildCompactionPrompt(messagesToCompact: Array<{ role: string; content: string }>): string {
  const body = messagesToCompact.map((m) => `[${m.role}]: ${m.content}`).join('\n\n')
  return `You are summarizing a coding conversation for continuity. The AI assistant will continue working after this summary replaces the original messages. Preserve ALL information needed to continue without re-asking the user.\n\nBe precise about: file paths, function names, error messages, decisions made and their rationale, current plan state, and what work remains.\n\n<messages_to_compact>\n${body}\n</messages_to_compact>\n\nRespond with a JSON object matching this schema:\n{ "compactionCount": <number>, "planState": "<string>", "keyDecisions": ["<string>"], "filesModified": ["<string>"], "errorsResolved": ["<string>"], "criticalContext": "<string>", "turnsSummarized": <number>, "originalTokenCount": <number> }`
}

export function parseCompactionResponse(response: string): CompactionSummary | null {
  try {
    // Try fenced code block first, then raw JSON
    ...
    return obj as CompactionSummary
  } catch {
    return null
  }
}
