...
function buildPreserveStoredValuesInput() {
  return {
    agentChatSettings: {
      defaultProvider: 'codex' as const,
      defaultVerificationProfile: 'full' as const,
      contextBehavior: 'manual' as const,
...
function buildPreserveStoredValuesExpected() {
  return {
...
function registerPreserveStoredValuesTest(): void {
  it('preserves stored agent chat defaults and existing Claude CLI settings', () => {
    expect(resolveAgentChatSettings(buildPreserveStoredValuesInput())).toEqual(
      buildPreserveStoredValuesExpected(),
    );
  });
}
