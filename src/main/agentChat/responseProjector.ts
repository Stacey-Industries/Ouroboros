...
export function mergeAdjacentTextBlocks(blocks: AgentChatContentBlock[]): AgentChatContentBlock[] {
  ...
  const transparentKinds = new Set<AgentChatContentBlock['kind']>(['thinking']);
  ...
  for (const block of blocks) {
    if (block.kind === 'text') {
      if (lastTextIdx >= 0) {
        ...
        // eslint-disable-next-line security/detect-object-injection -- lastTextIdx tracked from merged.length
...
function applyOptionalFields(
  ...
  if (args.orchestrationLink) message.orchestration = args.orchestrationLink;
  if (args.toolsUsed && args.toolsUsed.length > 0)
    message.toolsSummary = formatToolsSummary(args.toolsUsed);
  if (args.costUsd != null) message.costSummary = formatCost(args.costUsd);
...
function sealAndMergeBlocks(blocks: AgentChatContentBlock[]): AgentChatContentBlock[] {
  ...
  return mergeAdjacentTextBlocks(sealed);
}
...
export function projectProviderResultToAssistantMessage(
  ...
  applyOptionalFields(message, args);

  if (args.blocks && args.blocks.length > 0) {
    message.blocks = sealAndMergeBlocks(args.blocks);
  }
