...
export function createContextLayerWatcher(
  options: ContextLayerWatcherOptions,
): ContextLayerWatcher {
  ...
  return {
    ...
    forceRebuild: () => {
      if (!state.disposed) fireImmediateFullInvalidation(state, 'manual', emit);
    },
