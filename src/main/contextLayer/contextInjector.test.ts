...
describe('injectContextLayer', () => {
    ...
    for (let i = 0; i < 15; i++) {
      modules.push(
        createMockModuleEntry(`mod-${i}`, `src/mod${i}`, {
          ...
          ai: {
            description:
              `Module ${i} handles a very specific and detailed responsibility that uses up tokens in the budget estimation. `.repeat(
                3,
              ),
            keyResponsibilities: [
            ...
            gotchas: [
              `Gotcha for module ${i}: watch out for edge cases in serialization and token estimation`,
            ],
            generatedAt: 1000,
            generatedFrom: 'test',
            tokenCount: 200,
  ...
  it('returns repo map only when goalKeywords is empty', async () => {
    ...
    const enriched = result.packet as unknown as {
