...
describe('generateRepoMap', () => {
  it('generates a valid RepoMap with modules from RepoFacts and RepoIndexSnapshot', () => {
    ...
    const files = [
      createMockFile({
        relativePath: 'src/components/FileTree/FileTree.tsx',
        path: `${rootPath}/src/components/FileTree/FileTree.tsx`,
      }),
      createMockFile({
        relativePath: 'src/components/FileTree/index.ts',
        path: `${rootPath}/src/components/FileTree/index.ts`,
      }),
      createMockFile({
        relativePath: 'src/components/FileTree/utils.ts',
        path: `${rootPath}/src/components/FileTree/utils.ts`,
      }),
      createMockFile({
        relativePath: 'src/components/Terminal/Terminal.tsx',
        path: `${rootPath}/src/components/Terminal/Terminal.tsx`,
      }),
      createMockFile({
        relativePath: 'src/components/Terminal/index.ts',
        path: `${rootPath}/src/components/Terminal/index.ts`,
      }),
      createMockFile({
        relativePath: 'src/components/Terminal/hooks.ts',
        path: `${rootPath}/src/components/Terminal/hooks.ts`,
      }),
      createMockFile({
        relativePath: 'src/hooks/useConfig.ts',
        path: `${rootPath}/src/hooks/useConfig.ts`,
      }),
      createMockFile({
        relativePath: 'src/hooks/useTheme.ts',
        path: `${rootPath}/src/hooks/useTheme.ts`,
      }),
      createMockFile({
        relativePath: 'src/hooks/useFileWatcher.ts',
        path: `${rootPath}/src/hooks/useFileWatcher.ts`,
      }),
      createMockFile({
  ...
  it('enforces the 8KB size cap by truncating exports, imports, and module count', () => {
    ...
    mockedBuildSummaries.mockReturnValue(
      modules.map((mod, index) =>
        createMockStructuralSummary(mod, {
          exports: [...longExports],
          imports: [...longImports],
          fileCount: 40 - index,
        }),
...
describe('detectProjectName', () => {
  ...
  it('reads name from package.json when available', () => {
    ...
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({ name: 'ouroboros-ide' }) as unknown as string & Buffer<ArrayBufferLike>,
    );
  ...
  it('falls back to basename when package.json has empty name', () => {
    ...
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({ name: '' }) as unknown as string & Buffer<ArrayBufferLike>,
    );
...
describe('multi-root workspace', () => {
  it('prefixes module IDs with root basename when multiple roots exist', () => {
    ...
    const repoIndex: RepoIndexSnapshot = {
      ...
      roots: [createMockRootSnapshot(rootPath1, files1), createMockRootSnapshot(rootPath2, files2)],
    ...
    mockedBuildSummaries.mockReturnValue([
      createMockStructuralSummary(
        { ...frontendMod, id: 'frontend/app', label: 'frontend: App' },
        { fileCount: 1 },
      ),
      createMockStructuralSummary(
        { ...backendMod, id: 'backend/server', label: 'backend: Server' },
        { fileCount: 1 },
      ),
    ]);
    mockedBuildDeps.mockReturnValue([]);
