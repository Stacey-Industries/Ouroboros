...
export function buildModuleStructuralSummaries(options: {
  ...
  const files = options.files.map((f) => ({
    ...f,
    relativePath: normalizeSeparators(f.relativePath),
  }));
...
function processFileImports(ctx: FileImportCtx): void {
  ...
  for (const importSpecifier of imports) {
    ...
    if (existing) {
      ...
      edges.set(edgeKey, { from: moduleId, to: targetModule, weight: 1 });
    }
  }
}
...
function buildSummaryForModule(
  ...
  const totalLines = Math.ceil(
    moduleFiles.reduce((sum, f) => sum + f.size, 0) / AVERAGE_BYTES_PER_LINE,
  );
  ...
  const recentlyChanged = gitDiffFiles
    ? moduleFiles.some(
        (f) => gitDiffFiles.has(normalizeSeparators(f.relativePath)) || gitDiffFiles.has(f.path),
      )
    : false;
...
function extractExternalImports(files: IndexedRepoFile[]): string[] {
  ...
  for (const file of files) {
    for (const importSpec of file.imports) {
      ...
      const packageName =
        importSpec.startsWith('@') && parts.length >= 2 ? `${parts[0]}/${parts[1]}` : parts[0];
...
function computeContentHash(files: IndexedRepoFile[]): string {
  const entries = files.map(
    (f) => `${normalizeSeparators(f.relativePath).toLowerCase()}|${f.modifiedAt}`,
  );
  entries.sort();
