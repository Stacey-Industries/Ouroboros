...
function makeModuleEntry(
  overrides: Partial<ModuleContextEntry['structural']> = {},
): ModuleContextEntry {
  return {
    structural: {
      module: {
...
describe('contextLayerStore', () => {
  ...
  describe('readAllModuleEntries', () => {
    it('reads all module entries from the modules directory', async () => {
      await initContextLayerStore(testRoot);
      await writeModuleEntry(
        testRoot,
        'alpha',
        makeModuleEntry({
          module: { id: 'alpha', label: 'Alpha', rootPath: 'src/alpha', pattern: 'feature-folder' },
        }),
      );
      await writeModuleEntry(
        testRoot,
        'beta',
        makeModuleEntry({
          module: { id: 'beta', label: 'Beta', rootPath: 'src/beta', pattern: 'feature-folder' },
        }),
      );
      await writeModuleEntry(
        testRoot,
        'gamma',
        makeModuleEntry({
          module: { id: 'gamma', label: 'Gamma', rootPath: 'src/gamma', pattern: 'feature-folder' },
        }),
      );
