...
function makeContext(overrides: Partial<SummarizationContext> = {}): SummarizationContext {
  return {
    ...
    sourceSnippets: overrides.sourceSnippets ?? [
      {
        relativePath: 'src/renderer/components/FileTree/index.ts',
        content: 'export { FileTree } from "./FileTree"',
      },
      {
        relativePath: 'src/renderer/components/FileTree/FileTree.tsx',
        content: 'export function FileTree() { return <div /> }',
      },
    ],
...
const validSummaryBody = {
  description:
    'Renders a hierarchical file tree in the IDE sidebar, supporting expand/collapse and file selection.',
  keyResponsibilities: [
...
describe('summarizeModule', () => {
  ...
  it('returns rate_limited on 429 error without retrying', async () => {
    mockSpawnClaude.mockRejectedValueOnce(
      new Error('claude exited with code 1: 429 rate limit exceeded'),
    );
