...
async function safeDelete(workspaceRoot: string, moduleId: string): Promise<boolean> {
  try {
...
async function enforceModuleCount(
  ...
  if (result.deletedOverflow.length > 0) {
    log.info(
      '[context-layer] GC: module count enforced, deleted',
      result.deletedOverflow.length,
      'modules',
    );
  }
}
