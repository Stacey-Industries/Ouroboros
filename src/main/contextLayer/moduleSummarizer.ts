...
const ENTRY_POINT_NAMES = new Set(['index.ts', 'index.tsx', 'index.js', 'index.jsx']);

function addEntryPoints(files: SnippetFile[], moduleBasename: string, add: (rel: string) => void): void {
  for (const file of files) {
    const b = path.basename(file.relativePath).toLowerCase();
    if (ENTRY_POINT_NAMES.has(b) || b === `${moduleBasename}.ts` || b === `${moduleBasename}.tsx`) add(file.relativePath);
  }
}
...
function addTypeDefinitions(files: SnippetFile[], add: (rel: string) => void, maxSnippets: number, selected: SnippetResult[]): void {
  for (const file of files) {
    ...
    if (b.endsWith('.d.ts') || b.endsWith('types.ts') || b.endsWith('Types.ts')) add(file.relativePath);
  }
}
...
function buildUserMessage(
  ...
  const exportsLine =
    mod.exports.length > 0 ? mod.exports.slice(0, 10).join(', ') : 'no public exports';
...
const NETWORK_SIGNALS = ['ECONNREFUSED', 'ETIMEDOUT', 'fetch failed'];
const AUTH_SIGNALS = ['401', '403', 'Unauthorized', 'authentication'];

function classifyByMessage(message: string): string {
  if (NETWORK_SIGNALS.some((s) => message.includes(s))) return 'network_error';
  if (AUTH_SIGNALS.some((s) => message.includes(s))) return 'no_auth';
...
function buildSuccessResult(
  ...
  log.info(
    `[context-layer] Summarized module "${context.module.module.label}" (~${inputTokens} in, ~${outputTokens} out)`,
  );
  return {
    success: true,
    summary,
...
interface AttemptCtx { context: SummarizationContext; userMessage: string; startTime: number; extractedSymbols: ExtractedSymbol[]; attempt: number }

async function runAttempt(ctx: AttemptCtx): Promise<SummarizationResult | { lastError: string }> {
  const { context, userMessage, startTime, extractedSymbols, attempt } = ctx;
  const label = context.module.module.label;
  try {
    const { text, inputTokens, outputTokens } = await callCli(userMessage);
    const parsed = parseAndValidateResponse(text);
    if (parsed) return buildSuccessResult(parsed, context, { inputTokens, outputTokens, startTime }, extractedSymbols);
    if (attempt === 0) log.info(`[context-layer] Parse failed for "${label}", retrying... (first 300): ${text.slice(0, 300)}`);
    return { lastError: 'invalid_response' };
  } catch (error) {
    const classified = classifyError(error);
    if (classified === 'no_auth' || classified === 'rate_limited' || classified === 'network_error') {
      log.info(`[context-layer] CLI error for "${label}": ${classified}`);
      return { success: false, error: classified, durationMs: Date.now() - startTime, extractedSymbols };
    }
    if (attempt === 0) log.info(`[context-layer] Error for "${label}": ${classified}, retrying...`);
    return { lastError: classified };
  }
}
...
export async function summarizeModule(context: SummarizationContext): Promise<SummarizationResult> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    const result = await runAttempt({ context, userMessage, startTime, extractedSymbols, attempt });
    if ('success' in result) return result;
    lastError = result.lastError;
  }

  log.info(`[context-layer] Failed to summarize "${context.module.module.label}" after 2 attempts`);
  return {
    success: false,
