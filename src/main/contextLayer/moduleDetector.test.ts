...
describe('moduleDetector', () => {
  describe('detectModules', () => {
    ...
    it('detects single-file modules for standalone significant files', () => {
      const files = [makeFile({ relativePath: 'src/main/hooks.ts', size: 5000 })];
    ...
    it('does not create single-file modules for small files', () => {
      const files = [makeFile({ relativePath: 'src/main/tiny.ts', size: 100 })];
  ...
  describe('buildModuleStructuralSummaries', () => {
    ...
    it('collects unique languages from module files', () => {
      const files = [
        makeFile({
          relativePath: 'src/renderer/components/Mix/Component.tsx',
          language: 'typescript',
        }),
        makeFile({
          relativePath: 'src/renderer/components/Mix/styles.css',
          extension: '.css',
          language: 'css',
        }),
  ...
  describe('buildCrossModuleDependencies', () => {
    it('creates an edge when module A imports from module B via relative path', () => {
      ...
      const termToLayout = dependencies.find((d) => d.from === 'terminal' && d.to === 'layout');
    ...
    it('accumulates weight for multiple imports between the same modules', () => {
      ...
      const editorToShared = dependencies.find((d) => d.from === 'editor' && d.to === 'shared');
