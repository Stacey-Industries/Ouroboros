...
function handleToolsCall(id: JsonRpcId, params: Record<string, unknown>, toolDispatchMap: ToolDispatchMap): void {
...
async function connectServerEntry([name, serverConfig]: [string, McpServerConfig]): Promise<ConnectedUpstream> {
  const upstream = await connectUpstream(name, serverConfig)
  return { name, upstream }
}

function collectConnectedUpstreams(results: PromiseSettledResult<ConnectedUpstream>[]): Map<string, UpstreamServer> {
...
async function connectConfiguredUpstreams(servers: Record<string, McpServerConfig>): Promise<Map<string, UpstreamServer>> {
