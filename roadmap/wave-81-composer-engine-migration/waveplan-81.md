...
ADR file: `roadmap/wave-81-{slug}/wave-81-decisions.md`.
...
- Manual smoke entry in `roadmap/wave-81-{slug}/wave-81-auto-brief.md` per `~/.claude/rules/manual-smoke-gate.md`
...
| E | Auxiliary feature parity | sonnet-implementer | Image paste: custom Lexical paste plugin that intercepts clipboard items, calls existing `useImageAttachmentHandlers.handlePaste` for image files, lets text fall through. Drag-from-FileTree drop: custom drop handler on the ContentEditable container that parses the existing JSON dataTransfer payload and inserts a mention via `editor.update()` + `BeautifulMentionsPlugin`'s `insertMention` API. `agent-ide:quote-to-composer` event listener uses `editor.update()` + `TextNode` insertion to append text. Mid-turn inject button repositioning: review absolute-positioning anchor relative to ContentEditable's box (may differ from textarea's). |
| F | Cutover + cleanup | sonnet-implementer | Flip `VITE_LEXICAL_COMPOSER` default behavior: Lexical mounts unconditionally; legacy RichTextarea path deleted from `AgentChatComposerInput.tsx`. Remove `rich-textarea` from `package.json` + `package-lock.json`. Verify `grep -r "from 'rich-textarea'" src/renderer` returns empty. Rewrite `AgentChatComposerInput.test.tsx` to test Lexical surface (existing tests assert RichTextarea-specific behavior). Append a Lexical-migration gotcha entry to `src/renderer/components/AgentChat/CLAUDE.md`. Manual smoke + result brief at `roadmap/wave-81-{slug}/wave-81-auto-brief.md` per `~/.claude/rules/manual-smoke-gate.md`. |
...
| Custom slash-command plugin races with `BeautifulMentionsPlugin`'s `@` matcher (both run on every editor state change) | Phase D test suite includes a mixed `@user /clear` cursor-position case to verify both plugins coexist. Order plugin registration deterministically — slash plugin registers AFTER mention plugin to ensure `@` matcher runs first on shared text. |
...
| Manual smoke gate in chat-only window requires Cole to run dev build with flag set during phases B-E for verification | Document the env-var setting in Phase A audit. Phase F's manual smoke is the only blocking smoke; phases B-E observation is best-effort and acknowledged where it can't be done live. |
...
| B | `LexicalChatComposer.test.tsx` — keyboard plugin in isolation (Enter, Shift-Enter, Escape, ArrowUp, Tab) | n/a | Mount with empty editor; assert key behavior matches existing `handleComposerKeyDown` semantics. |
...
- [ ] Manual smoke checklist in `roadmap/wave-81-{slug}/wave-81-auto-brief.md` complete and signed.
...
12. `roadmap/wave-81-{slug}/phase-a-audit.md` — Phase A architect deliverable; required reading before phases B-F start.
13. `roadmap/wave-81-{slug}/wave-81-decisions.md` — locked architectural decisions for this wave.
...
1. Verify ADR exists at `roadmap/wave-81-{slug}/wave-81-decisions.md` with the six locked decisions filled in (decision 1 in full best-practice-spectrum form, others in abbreviated `Context / Pick / Rationale` form).
