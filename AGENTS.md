# Ouroboros — Codex Guidelines

## What This Is
Agent-first Electron desktop IDE for launching/monitoring Codex sessions. Three-process architecture (main → preload → renderer) with strict context isolation. **Built by Codex, running inside itself.** The agent makes implementation decisions autonomously; the human steers direction and reviews.

## Meta-Development Warning
**This project is developed from within itself.** Codex runs as a terminal session inside the Ouroboros — the very app being edited. This means:
...
| `src/main/hooks.ts` | Named pipe server for Codex events |
